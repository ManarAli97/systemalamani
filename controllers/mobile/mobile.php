...
class mobile extends Controller
{
    ...
    public function createTB()
    {
        ...
        $this->db->query("CREATE TABLE IF NOT EXISTS `{$this->cart_shop_active}` (
          `id` int(11)  NOT NULL AUTO_INCREMENT ,
        ...
        return $this->db->cht(array($this->table, $this->category, $this->color, $this->code, $this->excel, $this->cart_shop_active, $this->like_mobile, $this->comparison));
...
//								$p=(int)str_replace($this->comma,'',$split[0]);
    ...
    public function details($id)
    {
        ...
        $table=array();
        $stmt_table = $this->db->prepare("SELECT `{$this->color}`.*,  {$this->code}.size,  {$this->code}.code,   {$this->excel}.quantity  FROM `{$this->color}`  INNER JOIN {$this->code} ON {$this->code}.id_color = `{$this->color}`.id     INNER JOIN {$this->excel} ON {$this->excel}.code = `{$this->code}`.code    WHERE   `{$this->color}`.`id_item`= ? AND `{$this->color}`.`is_delete`= 0 AND  {$this->excel}.quantity > 0 ");
		...
		$savers=new  Savers();
            ...
            $idcarCover=array();
            $stmtIdCatCover=$this->db->prepare("SELECT id FROM type_device WHERE  id_device_mobile=? AND  `active` = 1 AND  {$this->is_delete} ");
			...
			$stmt = $this->db->prepare("SELECT * from `product_savers` WHERE `id_device` in({$idcarCover}) AND  {$this->is_delete} AND `active`=1 AND  `img` <> '' AND `title` <> '' ");
			$stmt->execute();
			if ($stmt->rowCount() > 0) {
...
//                    ->val('strip_tags');
                ...
                if (empty($this->error_form)) {
                    ...
                    $stmt = $this->db->prepare("INSERT INTO `{$this->table}` (`title`,`id_cat`,`content`,`description`,`date`,`bast_it`,`serial_flag`,`price_dollars`,`tags`,`cuts`,`price_cuts`,`specifications`,`location`,`enter_serial`,`id_device`,`stop`,`change_price`,`userId`,`is_service`) VALUE (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
                    $stmt->execute(array($data['title'], $data['id_cat'], $data['content'], $data['description'], $data['date'], $data['bast_it'], $data['serial_flag'], $data['price_dollars'], $data['tags'], $data['cuts'], $data['price_cuts'], $specifications, $data['location'], $data['enter_serial'],  $data['id_device'], $data['stop'], $data['change_price'],$this->userid,$data['is_service']));
...
//                                $stmt_cd_online->execute(array($cd, $size[$key][$index], $serial[$key][$index],  $lastId_c, time()));
      ...
      public function edit_mobile($id)
    {
        ...
        $stmt_last_id = $this->db->prepare("SELECT `id` FROM `{$this->color}` WHERE `id_item` = ? AND  {$this->is_delete} ORDER BY `id` DESC  LIMIT 1");
...
//                    ->val('strip_tags');
                ...
                if ($this->permit('save_edit', $this->folder)) {
                    if (empty($this->error_form)) {
                        $stmt = $this->db->prepare("UPDATE   `{$this->table}` SET  `title`=? ,  `content`=? ,  `description`=? , `id_cat`=? ,`date`=? ,`bast_it`=? ,`serial_flag`=? ,`price_dollars`=? ,`tags`=?,`cuts`=?,`price_cuts`=?,`specifications`=?,`location`=?,`enter_serial`=? ,`id_device`=? ,`stop`=?,`change_price`=? ,`userId`=?,`is_service`=? WHERE `id`=?");
                        $stmt->execute(array($data['title'], $data['content'], $data['description'], $data['id_cat'], $data['date'], $data['bast_it'], $data['serial_flag'], $data['price_dollars'], $data['tags'], $data['cuts'], $data['price_cuts'], $specifications, $data['location'], $data['enter_serial'],$data['id_device'],$data['stop'], $data['change_price'],$this->userid,$data['is_service'],$id));
...
// SQL server connection information
    ...
    function copy_row($id)
    {
        ...
        $stmt=$this->db->prepare("INSERT  INTO  mobile ( content, title, id_cat, id_main_cat, img, view, main_cat, date, bast_it, tags, specifications, cuts, price_cuts, description, name_accessories, id_accessories, name_savers, id_savers, serial_flag, price_dollars, location, enter_serial, stop, change_price, id_device, is_delete,active,userId)  SELECT  content, title, id_cat, id_main_cat, img, view, main_cat, date, bast_it, tags, specifications, cuts, price_cuts, description, name_accessories, id_accessories, name_savers, id_savers, serial_flag, price_dollars, location, enter_serial, stop, change_price, id_device, is_delete,0,$this->userid FROM mobile WHERE id=?");
        $stmt->execute(array($id));
        if ($stmt->rowCount() > 0)
        {
...
//  هنا صارت اضافة الايدي حتى ما ناثر ولا نتاثر باضافة الصورة
        ...
        if ($this->handleLogin()) {
            ...
            if ($stmt_item->rowCount() > 0) {
                while ($row_item = $stmt_item->fetch()) {
                    ...
                    $stmt_codes = $this->db->prepare("SELECT id,code FROM `code` where id_color in (select id from `color` where id_item =? )");
    ...
    {
        $stmt = $this->db->prepare("SELECT `id`, `id_item`,`size`,`price`,`price_dollars`,`image`,`color`,`name_color`,`code`,`table`,SUM(`number`)as number,`buy`,`date`,`offers`,`id_offer` FROM `{$this->cart_shop_active}` WHERE `id_member_r` =?  AND `buy` = 0 GROUP BY `code`, `table`,`number_bill`  ORDER BY `id`  DESC  ");
    ...
    function count_c()
    {
        ...
        $stmt = $this->db->prepare("SELECT `id`, `id_item`,`size`,`price`,`price_dollars`,`image`,`color`,`name_color`,`code`,`table`, `number`,`buy`,`date` FROM `{$this->cart_shop_active}` WHERE `id_member_r` =?  AND `buy` = 0 GROUP BY `offers`,`id_offer`,`date`,`date`  ORDER BY `id`  DESC  ");
        $stmt->execute(array($id));
        $car = array();
        $count = 0;
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
...
//    ------------edit buy---------
    ...
    {
        $stmt = $this->db->prepare("SELECT `id`, `id_item`,`size`,`price`,`price_dollars`,`image`,`color`,`name_color`,`code`,`table`,SUM(`number`)as number,`buy`,`date`,`date_req`,`mpx`,`number_bill`,`dollar_exchange` FROM `{$this->cart_shop_active}` WHERE `id_member_r` =?  AND   `buy` = 1 AND `status` =0  GROUP BY `id_item`,`table`,`code`,`color` ORDER BY `id` DESC  ");
        $stmt->execute(array($id_member_r));
        return $stmt;
    }
    ...
    {
        $stmt = $this->db->prepare("SELECT `id`, `id_item`,`size`,`price`,`price_dollars`,`image`,`color`,`name_color`,`code`,`table`,SUM(`number`)as number,`buy`,`date`,`date_req`,`mpx`,`dollar_exchange` FROM `{$this->cart_shop_active}` WHERE `id_member_r` =?  AND `buy` = 2  AND  `status`=1 GROUP BY `id_item`,`table`,`code`,`color`  ORDER BY `id` DESC  ");
        $stmt->execute(array($id_member_r));
        return $stmt;
    }
...
//    --------------------------
    ...
    public function getAllContentFromCarMost_req($id_member_r, $bill)
    {
        ...
        $stmt = $this->db->prepare("SELECT * FROM `{$this->cart_shop_active}` WHERE    `buy` = 2 AND `status` =1 AND `delivered` =0 AND `id_member_r`=? AND `number_bill`=?     ORDER BY `date_d_r` ASC ");
...
//    -----------delivery_user  in class register ---------
    ...
    {
        $stmt = $this->db->prepare("SELECT  `date_req`,`date_d_r`,`number_bill`,`id_member_r`  FROM `{$this->cart_shop_active}` WHERE `delivery_user` =?  AND   `buy` = 2 AND `status` =1 AND `delivered`=0 AND `delivery_user` <> 0 AND `number_bill` <> 0  GROUP BY `date_d_r`,`id_member_r`  ORDER BY `date_d_r` ASC ");
    ...
    {
        $stmt = $this->db->prepare("SELECT `id`, `id_item`,`size`,`price`,`price_dollars`,`image`,`color`,`name_color`,`code`,`table`,SUM(`number`)as number,`buy`,`date`,`date_d_r`,`mpx` FROM `{$this->cart_shop_active}` WHERE `id_member_r` =?  AND   `buy` = 2 AND `status` =1   AND `date_d_r`= ?  AND `delivery_user` =?  GROUP BY `id_item`,`table`,`code`,`date_d_r`,`id_member_r`  ORDER BY `id` DESC  ");
...
//    -----------delivery_user  in class register ---------
    ...
    public function getAllContentFromCar_details_client_done_prepared_requests()
    {
        ...
        if ($this->permit('customize_requests_for_delivery_staff', 'delivery_user')) {
            if (Session::get('role') == 'admin') {
                $stmt = $this->db->prepare("SELECT  `date_d_r`,`id_member_r`,`delivery_user`,`number_bill`  FROM `{$this->cart_shop_active}` WHERE    `buy` = 2 AND `status` =1 AND `delivered` =0  GROUP BY `number_bill`,`id_member_r`   ORDER BY `date_d_r` ASC ");
                $stmt->execute();
            } else {

                $stmt = $this->db->prepare("SELECT  `date_d_r`,`id_member_r`,`delivery_user`,`number_bill`  FROM `{$this->cart_shop_active}` WHERE    `buy` = 2 AND `status` =1 AND `delivered` =0 AND `delivery_user`=? GROUP BY `number_bill`,`id_member_r`   ORDER BY `date_d_r` ASC ");
       ...
       public function processing_quantity($id)
    {
        ...
        $columns = array(
            ...
            array('db' =>  'spare_code.spare_code', 'dt' =>6  ,
                'formatter' => function( $d, $row ) {
            ...
            array(
                'db' =>    'menu_link_device_acc_cover.name_device','dt' => 12  ),
...
// SQL server connection information
    ...
    function index_data($id)
    {
        ...
        $row['content']=array();
            ...
            $stmt_content = $this->getAllmobileFromContent($this->getLoopId($id), 8);
    ...
    function convert_spar_code($model)
    {
        ...
        if ($model == 'accessories') {
            ...
            $stmt = $this->db->prepare("SELECT *FROM  color_accessories WHERE  serial <> '' ");
            $stmt->execute();
            echo 'model=code=spare_code <br>';
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {

                    $serial = explode(',', $row['serial']);
                    foreach ($serial as $spare_code) {

                        $stmtCh = $this->db->prepare("SELECT  * FROM spare_code  WHERE  code=?  AND spare_code = ?  AND  model =?  AND  id_item =? ");
                        $stmtCh->execute(array($row['code'], $spare_code, $model, $row['id_item']));
                        if ($stmtCh->rowCount() <= 0) {
                            ...
                            $stmtIn->execute(array($row['code'], $row['color'], $row['size'], $spare_code, $model, $this->userid, time(), $row['id_item']));
                            if ($stmtIn->rowCount() > 0) {
                                echo $model . '=' . $row['code'] . '=' . $spare_code . '<br>';
                            }
                        }
                    }
            ...
            $stmt = $this->db->prepare("SELECT *FROM  product_savers WHERE  serial <> '' ");
            $stmt->execute();
            echo 'model=code=spare_code <br>';
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            $stmt = $this->db->prepare("SELECT *FROM {$code} WHERE  serial <> '' ");
            $stmt->execute();
            echo 'model=code=spare_code <br>';
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                $stmtColor = $this->db->prepare("SELECT  * FROM {$color} WHERE  id =  ?  LIMIT 1");
                $stmtColor->execute(array($row['id_color']));
                if ($stmtColor->rowCount() > 0) {

                    $colorTable = $stmtColor->fetch(PDO::FETCH_ASSOC);
                    $serial = explode(',', $row['serial']);
                    foreach ($serial as $spare_code) {

                        $stmtCh = $this->db->prepare("SELECT  * FROM spare_code  WHERE  code=?  AND spare_code = ?  AND  model =?  AND  id_item =? ");
                        $stmtCh->execute(array($row['code'], $spare_code, $model, $colorTable['id_item']));
                        if ($stmtCh->rowCount() <= 0) {
                            $stmtIn = $this->db->prepare("INSERT INTO spare_code ( code, color, size, spare_code, model, userId, date,id_item) VALUES (?,?,?,?,?,?,?,?) ");
                            $stmtIn->execute(array($row['code'], $colorTable['color'], $row['size'], $spare_code, $model, $this->userid, time(), $colorTable['id_item']));
                            if ($stmtIn->rowCount() > 0) {
                                echo $model . '=' . $row['code'] . '=' . $spare_code . '<br>';
                            }
                        }
