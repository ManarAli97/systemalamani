...
class truth_screen_questions extends Controller
{
    ...
    {
        parent::__construct();
        $this->table='truth_screen_questions';
        $this->menu=new Menu();
    }
    ...
    public function list_truth_screen_questions()
    {
        $this->checkPermit('list_truth_screen_questions','truth_screen_questions');
        $this->adminHeaderController($this->langControl('truth_screen_questions'));
    ...
    $columns = array(
        ...
        array(
            ...
            'formatter' => function($id, $row ) {
                if ($this->permit('visible','truth_screen_questions')) {
                    return "
        ...
        array(
            ...
            'dt'        => 4,
            'formatter' => function($id, $row ) {
                if ($this->permit('edit','truth_screen_questions')) {
                    return "
        ...
        array(
            ...
            'dt'        => 5,
            'formatter' => function($id, $row ) {
                if ($this->permit('delete','truth_screen_questions')) {
                    return "
                <div style='text-align: center'>
...
// SQL server connection information
    ...
    public function visible_truth_screen_questions($v_,$id_)
    {
        if ($this->handleLogin()) {
    ...
    function delete_truth_screen_questions($id)
    {
        if ($this->handleLogin() ) {
    ...
    function add()
    {
        $this->checkPermit('add','truth_screen_questions');
        ...
        if (isset($_POST['submit']))
        {
            try{
                ...
                $this->lightRedirect(url.'/'.$this->folder.'/list_truth_screen_questions',0);
            }
...
//                     }
    ...
    public function getAlltruth_screen_questions()
    {
    ...
    public  function view_truth_screen_questions($page=1)
   {
    ...
    while ($row =$stmt->fetch(PDO::FETCH_ASSOC))
    {
        ...
        $truth_screen_questions[]=$row;
    }

    if (!empty($truth_screen_questions))
    {
        $truth_screen_questions=array_chunk($truth_screen_questions,8);
        $count=count($truth_screen_questions);
        $truth_screen_questions=$truth_screen_questions[$page-1];
    }
