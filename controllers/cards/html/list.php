...
<script>
    $(document).ready(function() {
        ...
        $('#example').DataTable( {
            "processing": true,
            "serverSide": true,
            "ajax": "<?php echo url ?>/cards/processing/<?php echo $id  ?>",
            info:false,
...
<div class="row">
    <div class="col-lg-3">
            <select class="custom-select custom-select-sm" onchange="location = this.value;">
                ...
                <option value="<?php echo url ?>/cards/list_cards/<?php   echo $list_cat['id'] ?>"  <?php   echo $list_cat['selected'] ?>   > <?php   echo $list_cat['title'] ?> </option>
                <?php  }  ?>
            </select>
    </div>
    ...
    <div class="col-lg-9">
        ...
        <a  href="<?php echo url ?>/cards/add_cards/<?php echo $id ?>" role="button"    class="btn btn-primary btn-sm"> <i class="fa fa-plus" aria-hidden="true"></i>  <span><?php  echo $this->langControl('add_content') ?>  </span> </a>
        <a  href="<?php  echo url  ?>/cards/admin_category"  role="button"   class="btn btn-warning btn-sm"> <i class="fa fa-list" aria-hidden="true"></i> <span><?php  echo $this->langControl('show_category') ?> </span></a>
        <a  href="<?php  echo url  ?>/cards/add_category"  role="button"   class="btn btn-info btn-sm">  <i class="fa fa-folder-open-o" aria-hidden="true"></i>  <span><?php  echo $this->langControl('add_category') ?>   </span></a>
...
<script>
    ...
    $('#save').on('click',function () {
        var  id= $('#save').val();
        $.get( "<?php echo url ?>/cards/delete_cards/"+id, function( data ) {
            $('#row_'+id).remove();
...
<script>
    ...
    function visible_cards(e,id) {
        var vis=$(e).is( ':checked' )? 1:0;
        $.get("<?php echo url ?>/cards/visible_cards/"+vis+'/'+id, function(){ })
    }
