...
<div class="row align-items-center">
    <div class="col">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="<?php  echo url.'/'.$this->folder?>"><?php  echo $this->langControl('tamayaz_locations') ?> </a></li>
...
<div class="row">
    <div class="col-lg-9">
        <a  href="<?php echo url .'/'.$this->folder   ?>/add_tamayaz_locations" role="button"    class="btn btn-primary btn-sm"> <i class="fa fa-plus" aria-hidden="true"></i>  <span><?php  echo $this->langControl('add_content') ?>  </span> </a>
...
<script>
    $(document).ready(function() {
        ...
        $('#example').DataTable( {
            "processing": true,
            "serverSide": true,
            "ajax": "<?php echo url .'/'.$this->folder ?>/processing_tamayaz_locations",
            info:false,
...
<script>
    ...
    $('#save').on('click',function () {
        var  id= $('#save').val();
        $.get( "<?php echo url .'/'.$this->folder ?>/delete_tamayaz_locations/"+id, function( data ) {
            $('#row_'+id).remove();
...
<script>
    ...
    function visible_tamayaz_locations(e,id) {
        var vis=$(e).is( ':checked' )? 1:0;
        $.get("<?php echo url .'/'.$this->folder ?>/visible_tamayaz_locations/"+vis+'/'+id, function(){ })
    }
...
<div class="modal fade" id="exampleModal_edit" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="edit_tamayaz_locations" action="" method="post">
...
<script>
    $('#exampleModal_edit').on('show.bs.modal', function (event) {
        ...
        var modal = $(this);
        $.ajax({
            url: "<?php  echo url . '/' . $this->folder?>/get_tamayaz_locations/"+id,
            cache: false,
            success: function(data){
                if (data)
                {
                    ...
                    modal.find('#edit_tamayaz_locations').attr("action","<?php  echo url .'/'.$this->folder?>/edit_tamayaz_locations/"+id);
                }
            }
        });
    });
    ...
    $(function () {
        $('#edit_tamayaz_locations').on('submit', function (e) {
            e.preventDefault();
            data = $('#edit_tamayaz_locations').serialize()
            $.ajax({
                type: 'post',
                url: this.action,
                data: data,
