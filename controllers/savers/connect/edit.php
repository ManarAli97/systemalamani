...
<form action="<?php echo url.'/'.$this->folder ?>/edit_savers_connect/<?php echo $id ?>" method="post">
...
<div class="row">
    <div class="col-lg-6 col-md-6 col-sm-12">
        <div class="row">
            <div class="col-12">
                <label  > <span>    بحت عن اجهزة متشابهة في الحافظات  </span>   </label>
            </div>
            <div class="col-12">
    ...
    <div class="col-lg-6 col-md-6 col-sm-12">
        <div class="customer_lest">
            <?php
            for ($i=0 ; $i<count($title) ; $i++) { ?>
                <div class="custom-control custom-checkbox custom-control-inline listCustom-checkbox" id="remove_<?php echo $ids[$i] ;?>">
                    <input type="checkbox" id="customRadioInline_<?php echo $ids[$i] ;?>" value="<?php echo $ids[$i] ;?>" name="ids[<?php echo $ids[$i] ;?>]" class="custom-control-input" checked>
                    <label class="custom-control-label" for="customRadioInline_<?php echo $ids[$i] ;?>"> <span> <?php echo $title[$i] ;?> </span>  <button  class='btn' type="button" onclick="remove_this_costom(<?php echo $ids[$i] ;?>)">   <i class='fa fa-times-circle'></i> </button></label>
                </div>
            <?php }    ?>
        </div>
    </div>
...
<script>
    ...
    function showResult(str) {
        ...
        xmlhttp.open("GET","<?php echo url.'/'.$this->folder ?>/search/<?php  echo $id ?>?q="+str,true);
        xmlhttp.send();
    }

    function add_c(id) {
        $.get( "<?php echo url.'/'.$this->folder ?>/savers_info/"+id, function( data ) {
