...
<div class="row align-items-center">
    <div class="col">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="<?php  echo url.'/'.$this->folder?>"><?php  echo $this->langControl('print_devices') ?> </a></li>
...
<div class="row">
    <div class="col-lg-9">
        <a  href="<?php echo url .'/'.$this->folder   ?>/add_print_devices" role="button"    class="btn btn-primary btn-sm"> <i class="fa fa-plus" aria-hidden="true"></i>  <span><?php  echo $this->langControl('add_content') ?>  </span> </a>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        ...
        $('#example').DataTable( {
            "processing": true,
            "serverSide": true,
            "ajax": "<?php echo url .'/'.$this->folder ?>/processing_print_devices",
            info:false,
...
<script>
    ...
    $('#save').on('click',function () {
        var  id= $('#save').val();
        $.get( "<?php echo url .'/'.$this->folder ?>/delete_print_devices/"+id, function( data ) {
            $('#row_'+id).remove();
...
<script>
    ...
    function visible_print_devices(e,id) {
        var vis=$(e).is( ':checked' )? 1:0;
        $.get("<?php echo url .'/'.$this->folder ?>/visible_print_devices/"+vis+'/'+id, function(){ })
    }
...
<div class="modal fade" id="exampleModal_edit" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="edit_print_devices" action="" method="post">
...
<script>
    $('#exampleModal_edit').on('show.bs.modal', function (event) {
        ...
        var modal = $(this);
        $.ajax({
            url: "<?php  echo url . '/' . $this->folder?>/get_print_devices/"+id,
            cache: false,
            success: function(data){
                if (data)
                {
                    ...
                    modal.find('#edit_print_devices').attr("action","<?php  echo url .'/'.$this->folder?>/edit_print_devices/"+id);
                }
            }
        });
    });
    ...
    $(function () {
        $('#edit_print_devices').on('submit', function (e) {
            e.preventDefault();
            data = $('#edit_print_devices').serialize()
            $.ajax({
                type: 'post',
                url: this.action,
                data: data,
