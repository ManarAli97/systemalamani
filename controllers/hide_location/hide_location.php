...
class hide_location extends Controller
{
    ...
    public function list_hide_location()
    {
        $this->checkPermit('list_hide_location', 'hide_location');
        $this->adminHeaderController($this->langControl('hide_location'));
    ...
    public function add_hide_location()
    {
        $this->checkPermit('add', 'hide_location');
        $this->adminHeaderController($this->langControl('add'));
        $this->AddToTraceByFunction($this->userid,'hide_location','add_hide_location');
        ...
        if (isset($_POST['submit'])) {
            try {
                ...
                foreach ($title as $key => $save_data) {
                    $stmt_s = $this->db->prepare("INSERT INTO `{$this->table}` (`location`,`userid`) VALUES (?,?)");
                    $stmt_s->execute(array(trim($save_data),$this->userid));
                }


                $this->lightRedirect(url . '/' . $this->folder . '/list_hide_location', 0);
            } catch (Exception $e) {
                $data = $form->fetch();
    ...
    public function processing_hide_location()
    {
        ...
        $columns = array(
            ...
            array(
                ...
                'formatter' => function ($id, $row) {
                    if ($this->permit('delete', 'hide_location')) {
                        return "
...
// SQL server connection information
    ...
    public function ch_hide_location($id)
    {
    ...
    public function visible_hide_location($v_, $id_)
    {
        if ($this->handleLogin()) {
    ...
    function delete_hide_location($id)
    {
        if ($this->handleLogin()) {
            $this->AddToTraceByFunction($this->userid,'hide_location','delete_hide_location/'.$id);
    ...
    public function edit_hide_location($id = null)
    {
    ...
    function  excel()
    {
        ...
        $this->AddToTraceByFunction($this->userid,'hide_location','excel');
        ...
        if(isset($_POST["submit"])) {
            ...
            try {
                ...
                if (file_exists($inputFileName)) {
                    ...
                    for ($row = 1; $row <= $highestRow; $row++) {
                        ...
                        if (count($rowData[0])  >= 1)
                        {
                            ...
                            $stmt=$this->db->prepare("INSERT INTO `hide_location` (`location`,`userid`) VALUES (?,?)");
                ...
                {
                    $this->lightRedirect(url.'/'.$this->folder."/list_hide_location");
