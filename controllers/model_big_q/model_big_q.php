...
class model_big_q extends Controller
{
    ...
    public function list_model_big_q()
    {
        $this->checkPermit('list_model_big_q', 'model_big_q');
        $this->adminHeaderController($this->langControl('model_big_q'));
    ...
    public function add_model_big_q()
    {
        $this->checkPermit('add', 'model_big_q');
        ...
        if (isset($_POST['submit'])) {
            try {
                ...
                $title = json_decode($data['code'], true);
                $model = json_decode($data['model'], true);


                foreach ($title as $key => $save_data) {
                    $stmt_s = $this->db->prepare("INSERT INTO `{$this->table}` (`code`,`model`,`userid`) VALUES (?,?,?)");
                    $stmt_s->execute(array(trim($save_data),$model[$key],$this->userid));
                }


                $this->lightRedirect(url . '/' . $this->folder . '/list_model_big_q', 0);
            } catch (Exception $e) {
                $data = $form->fetch();
    ...
    public function processing_model_big_q()
    {
        ...
        $columns = array(
            ...
            array(
                ...
                'formatter' => function ($id, $row) {
                    if ($this->permit('delete', 'model_big_q')) {
                        return "
...
// SQL server connection information
    ...
    public function ch_model_big_q($id)
    {
    ...
    public function visible_model_big_q($v_, $id_)
    {
        if ($this->handleLogin()) {
    ...
    function delete_model_big_q($id)
    {
        if ($this->handleLogin()) {
    ...
    public function edit_model_big_q($id = null)
    {
        ...
        if ($this->handleLogin()) {
            ...
            $title = $_POST['code'];
            $stmt = $this->db->update($this->table, array('code' => $title), "id={$id}");
