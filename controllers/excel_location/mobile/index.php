...
<div class="row">
	<div class="col">
		...
		<nav aria-label="breadcrumb">
			<ol class="breadcrumb">
				<li class="breadcrumb-item"><a href="<?php  echo url.'/'.$this->folder?>/mobile"><?php  echo $this->langControl($this->folder) ?> </a></li>
				<li class="breadcrumb-item active" aria-current="page" >  <?php  echo $this->langControl('location_mobile') ?></li>
			</ol>
		</nav>
...
<script>
    $(document).ready(function() {
        ...
        var t=$('#example').DataTable( {
            "processing": true,
            "serverSide": true,
            "ajax": "<?php echo url .'/'. $this->folder ?>/processing/mobile",
            info:false,
...
<div class="row justify-content-between">
	<div class="col-auto">
		<a class="btn btn-primary" href="<?php  echo url .'/'.$this->folder ?>/add/mobile"> <i class="fa fa-plus"> <span>  رفع Excel </span></i>     </a>
	</div>
	<div class="col-auto">
		<?php 	if ($this->permit('mobile_delete_all_location', $this->folder)) { ?>
			<div class="text-left">
				<button class="btn btn-danger" onclick="delete_all('mobile')"> <i class="fa fa-trash"></i> <span>حذف الكل</span></button>
			</div>
...
<form id="idForm" action="<?php  echo url .'/'.$this->folder ?>/lct/mobile" method="post">
	<div class="row align-items-end">
		...
		<div class="col-auto">
			...
			<input style="width: 150px"   onkeyup="get_info_code(this)" type="text" class="xdata form-control" name="code" required autocomplete="off">
		</div>
...
<script>
    ...
    function get_info_code(e) {
        var code= $(e).val()
        if (code) {
            $.get("<?php   echo url . './' . $this->folder?>/get_info_code/mobile", {code: code}, function (data) {
                $(".result").html(data);
            });
        }else
        {
        ...
        $.ajax({
            ...
            success: function(data)
            {
                ...
                if (data==='1')
                {
                    ...
                    alert('  القسم لا يحتوى على مواقع  ')
