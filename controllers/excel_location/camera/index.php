...
<div class="row">
	<div class="col">
		...
		<nav aria-label="breadcrumb">
			<ol class="breadcrumb">
				<li class="breadcrumb-item"><a href="<?php  echo url.'/'.$this->folder?>/camera"><?php  echo $this->langControl($this->folder) ?> </a></li>
				<li class="breadcrumb-item active" aria-current="page" >  <?php  echo $this->langControl('location_camera') ?></li>
			</ol>
		</nav>
...
<script>
    $(document).ready(function() {
        ...
        var t=$('#example').DataTable( {
            "processing": true,
            "serverSide": true,
            "ajax": "<?php echo url .'/'. $this->folder ?>/processing/camera",
            info:false,
...
<div class="row justify-content-between">
	<div class="col-auto">
		<a class="btn btn-primary" href="<?php  echo url .'/'.$this->folder ?>/add/camera"> <i class="fa fa-plus"> <span>  رفع Excel </span></i>     </a>
	</div>
	<div class="col-auto">
		<?php 	if ($this->permit('camera_delete_all_location', $this->folder)) { ?>
			<div class="text-left">
				<button class="btn btn-danger" onclick="delete_all('camera')"> <i class="fa fa-trash"></i> <span>حذف الكل</span></button>
			</div>
...
<form id="idForm" action="<?php  echo url .'/'.$this->folder ?>/lct/camera" method="post">
...
<script>
    ...
    function get_info_code(e) {
        var code= $(e).val()
        if (code) {
            $.get("<?php   echo url . './' . $this->folder?>/get_info_code/camera", {code: code}, function (data) {
                $(".result").html(data);
            });
        }else
        {
