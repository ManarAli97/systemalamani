...
class bill_secondary_accountant extends Controller
{
    ...
    {
        $this->checkPermit('bill_secondary_accountant', $this->folder);
        $this->adminHeaderController($this->langControl('bill_secondary_accountant'));
        ...
        if ($stmt_groups->rowCount() > 0)
        {
             while ($result_g=$stmt_groups->fetch(PDO::FETCH_ASSOC)) {
              ...
              /*  if ($this->admin($this->userid)) {
                    ...
                    $stmt = $this->db->prepare("SELECT *FROM  `user` WHERE  `idGroup`  = ?  AND id =?  AND `direct`=3");
                    $stmt->execute(array($result_g['id'], $this->userid));
                }
*/

                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    $row['money'] = number_format($this->sumDiscount($row['id']));
...
// SQL server connection information
	...
	function sumDiscount($id)
	{
		if ($this->handleLogin())
		{
				...
				$stmt = $this->db->prepare("SELECT    sum(`money`) as money  FROM `log_accountant` WHERE  id_user=?  ");
				$stmt->execute(array($id));
				$result = $stmt->fetch(PDO::FETCH_ASSOC);
				...
				return $result['money'] + $result1['money'];
