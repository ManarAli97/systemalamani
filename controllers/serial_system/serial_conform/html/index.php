...
<script>
    $(document).ready(function() {
        ...
        var table= $('#example').DataTable( {
            "processing": true,
            "serverSide": true,
            "ajax": "<?php echo url .'/'.$this->folder ?>/processing_serial_conform/<?php  echo $model ?>",
            info:false,
        ...
        <div class="col-auto">
            <select name="model"  id="her_add_menu" class="custom-select list_menu_categ" onchange="location=this.value"  >
                <?php  foreach ($this->category_website as $key => $cg) {   ?>
                    <option <?php  if ($key==$model) echo 'selected'?>  value="<?php  echo url .'/'.$this->folder ?>/list_serial_conform?model=<?php  echo $key ?>"     > <?php  echo $cg ?></option>
                <?php  } ?>
            </select>
