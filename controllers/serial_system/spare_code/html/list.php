...
<script>
    $(document).ready(function() {
        ...
        var table= $('#example').DataTable( {
            "processing": true,
            "serverSide": true,
            "ajax": "<?php echo url .'/'.$this->folder ?>/processing_spare_code/<?php  echo $model ?>",
            info:false,
        ...
        <div class="col-auto">
            <select name="model"  id="her_add_menu" class="custom-select list_menu_categ" onchange="location=this.value"  >
                <?php  foreach ($this->category_website as $key => $cg) {   ?>
                    <option <?php  if ($key==$model) echo 'selected'?>  value="<?php  echo url .'/'.$this->folder ?>/list_spare_code?model=<?php  echo $key ?>"     > <?php  echo $cg ?></option>
                <?php  } ?>
            </select>

        </div>
        <div class="col-auto">
            <a  href="<?php echo url .'/'.$this->folder?>/add_spare_code?model=<?php   echo  $model    ?>" role="button"    class="btn btn-primary"> <i class="fa fa-plus-circle" aria-hidden="true"></i>  <span> اضافة باركودات بديلة قسم   </span>  <span><?php  echo $this->langControl($model) ?></span> </a>
        </div>

        <div class="col-auto">
            <a  href="<?php echo url .'/'.$this->folder?>/excel/<?php   echo  $model    ?>" role="button"    class="btn btn-warning"> <i class="fa fa-file-excel-o" aria-hidden="true"></i>  <span>   رفع اكسيل باركودات بديلة </span>  <span><?php  echo $this->langControl($model) ?></span> </a>
        </div>
