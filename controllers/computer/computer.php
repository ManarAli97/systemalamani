...
class computer extends Controller
{
    ...
    function __construct()
    {
        parent::__construct();
        $this->table='computer';
        $this->category='category_computer';
        $this->color='color_computer';
        $this->code='code_computer';
        $this->excel='excel_computer';
        $this->cart_shop_active='cart_shop_active';
        $this->like_computer='like_computer';
        $this->menu=new Menu();
        $this->setting=new Setting();
        $this->category_connect = "category_{$this->folder}_connect";
        $this->class_computer='class_computer';
    }
    ...
    public function createTB()
    {
        ...
        $this->db->query("CREATE TABLE IF NOT EXISTS `{$this->table}` (
          ...
          `date` bigint(20) NOT NULL,
           `class_computer` varchar(250) COLLATE utf8_unicode_ci NOT NULL,
           PRIMARY KEY (`id`)
     ) ENGINE=InnoDB AUTO_INCREMENT=0 DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci");
        ...
        $this->db->query("CREATE TABLE IF NOT EXISTS `{$this->like_computer}` (
          `id` int(11)  NOT NULL AUTO_INCREMENT ,
        ...
        $this->db->query("CREATE TABLE IF NOT EXISTS `{$this->class_computer}` (
           `id` int(10) NOT NULL AUTO_INCREMENT ,
           `title` varchar(250) COLLATE utf8_unicode_ci NOT NULL,
           `active` int(10) NOT NULL DEFAULT '0',
        ...
        return  $this->db->cht(array($this->table, $this->category, $this->color, $this->code, $this->excel,$this->like_computer,$this->class_computer));
    ...
    public function admin_category($id=0)
    {
        ...
        $this->checkPermit('View_category','computer');
    ...
    public function list_computer($id=null)
    {
        if (!is_numeric($id)) {$error=new Errors(); $error->index();}
        $this->checkPermit('view_content','computer');
    ...
    public  function list_view($id=null,$page=1)
    {
        ...
        $class_computer=$this->db->select("SELECT * from  {$this->class_computer} WHERE `active`=1");
    ...
    public function details($id)
    {
        ...
        while ($row=$stmtc->fetch(PDO::FETCH_ASSOC))
        {
            ...
            $stmtco=$this->db->prepare("SELECT *FROM {$this->code} WHERE `id_color`=?  AND  {$this->is_delete} ");
    ...
    function dtl_start_price($id)
    {
        ...
        $stmt_c= $this->db->prepare("SELECT `id` from `{$this->color}` WHERE  `id_item`=?  AND  {$this->is_delete} ");
        ...
        $stmt= $this->db->prepare("SELECT `code` from `{$this->code}` WHERE  `id_color` IN ({$Id_color})  AND  {$this->is_delete} ");
        $stmt->execute(array($id));
        $code=array();
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC))
        {
    ...
    public function add_computer($id=null,$r=null)
    {

        if (!is_numeric($id)) {$error=new Errors(); $error->index();}
        $this->checkPermit('add','computer');
        ...
        $class_computer=$this->db->select("SELECT * from  {$this->class_computer} ");
        $stmt_specif=$this->db->prepare("SELECT *FROM `specifications`  WHERE `model`=?");
        $stmt_specif->execute(array($this->folder));
        $specifications=array();
        while ($row = $stmt_specif->fetch(PDO::FETCH_ASSOC))
        ...
        if (isset($_POST['submit']))
        {
            try
            {
                ...
                $class_computer=json_decode($data['class_computer'],true);
                ...
                if (!empty($data['class_computer'])) {
                    $class_computer = json_decode($data['class_computer'], true);
                    $class_computer = implode(',', $class_computer);
                }else{
                    $class_computer='';
                }
                ...
                if (empty($this->error_form)) {
                    ...
                    $stmt = $this->db->prepare("INSERT INTO `{$this->table}` (`title`,`id_cat`,`content`, `description` ,`date`,`bast_it`,`serial_flag`,`price_dollars`,`tags`,`class_computer`,`cuts`,`price_cuts`,`specifications`,`location`,`enter_serial`,`change_price`,`userId`,`is_service`) VALUE (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
                    $stmt->execute(array($data['title'], $data['id_cat'], $data['content'], $data['description'], $data['date'], $data['bast_it'], $data['serial_flag'], $data['price_dollars'], $data['tags'], $class_computer, $data['cuts'], $data['price_cuts'], $specifications, $data['location'], $data['enter_serial'], $data['change_price'],$this->userid,$data['is_service']));
                    ...
                    if ($r)
                    {
                        ...
                        $this->lightRedirect(url . "/computer/list_computer/{$id}", 0);
                    }
                }
            }catch (Exception $e)
            {
    ...
    {
        $this->checkPermit('edit', 'computer');
        if (!is_numeric($id)) {
            $error = new Errors();
            $error->index();
        }
        ...
        $clgame_id=explode(',',$data['class_computer']);
            ...
            try
            {
               ...
               $form->post('is_service')
                ...
                if ($this->permit('save_edit',$this->folder)) {
                    ...
                    $stmt = $this->db->prepare("UPDATE   `{$this->table}` SET  `title`=? ,  `content`=? , `description`=? , `id_cat`=? ,`date`=? ,`bast_it`=? ,`serial_flag`=? ,`price_dollars`=?,`tags`=?,`class_computer`=?,`cuts`=?,`price_cuts`=?,`specifications`=?,`location`=?,`enter_serial`=? ,`change_price`=? ,`userId`=?,`is_service`=? WHERE `id`=?");
                    $stmt->execute(array($data['title'], $data['content'],$data['description'], $data['id_cat'], $data['date'], $data['bast_it'], $data['serial_flag'], $data['price_dollars'], $data['tags'], $class_computer, $data['cuts'], $data['price_cuts'], $specifications, $data['location'],$data['enter_serial'],$data['change_price'],$this->userid,$data['is_service'],$id));
                    ...
                    $this->lightRedirect(url . "/computer/list_computer/{$data['id_cat']}", 0);
        ...
        $columns = array(
            ...
            array(
                ...
                <div style='text-align: center'>
                  <input {$this->ch($id)} class='toggle-demo' onchange='visible_computer(this,$id)' type='checkbox' data-on='On' data-off='Off' id='toggle-event'    data-toggle='toggle' data-style='ios' data-onstyle='success' data-size='small'>
                 </div>
             ";  }
                    else
                    {
...
// SQL server connection information
    ...
    public function visible_computer($v_,$id_)
    {
        if ($this->handleLogin()) {
    ...
    public function select_category_computer($id)
    {
...
//-----------------category------------------
    ...
    {
        $this->checkPermit('add_category','computer');
        if (!is_numeric($id)){  $error=new Errors();  $error ->index();}
        $breadcumbs = $this ->Breadcumbs( $this->category,$id);
        $this->adminHeaderController($this->lang('computer'),$id);
        $data['title']='';
                $data['code_cat']='';
...
//  هنا صارت اضافة الايدي حتى ما ناثر ولا نتاثر باضافة الصورة
				$this->Add_to_sync_schedule($id_cat_h,'computer','add_category');
    ...
    function edit_category($id)
    {
        ...
        $this->checkPermit('edit_category','computer');
...
//                 h27
					$this->Add_to_sync_schedule($id,'computer','add_category');
                    $this->lightRedirect(url . '/' . $this->folder . "/admin_category/{$id}", 0);
                }
            }
            catch (Exception $e)
    ...
    function delete($id)
    {
        ...
        if ($this->handleLogin()) {
            ...
            $this->Add_to_sync_schedule($id, 'computer', 'delete_category');
        }
    }
    ...
    public function select_category_computer_public($id)
    {
    ...
    public function select_category_computer_publicAll()
    {
    ...
    public function listSubCategoryMenu_acc($relid)
    {
        ...
        while ($row = $stmt->fetch()) {
            ...
            if ($row['img'] !=0) {
                $get_file = $this->db->select("SELECT * from `files` WHERE `id`=:id AND `module`=:module LIMIT 1 ", array(':id' => $row['img'], ':module' => 'computer_cat'));
                $get_file = $get_file[0];
                $row['image'] ='<img class="ion_catg" src="'.$this->save_file.$get_file['rand_name'].'">';

            } else
            {
                $row['image']= '<i class="fa fa-video-computer"></i>';
            }
                        ...
                        <div class="card-header" id="headingOne1<?php echo $row['id'] ?>">
                            <div class="row justify-content-between align-items-center">
                                <div class="col-<?php if ($this->ck_sub_cat_public($row['id'])) echo 8;  else echo 12 ;?>">
                                    <a  class="list_link" href="<?php  echo url ?>/computer/list_view/<?php echo $row['id'] ?>" >
                                        <div class="row align-items-center">
                                            <div class="col-auto" style="padding-left: 3px" ><?php  echo $row['image']  ?> </div>
                                            <div class="col-8" style="padding-right: 3px;padding-left: 1px" > <?php echo $row['title'] ?> </div>
                                        </div>
                                    </a>
                                </div>
                            ...
                            <div class="row justify-content-between align-items-center">
                                <div class="col-<?php if ($this->ck_sub_cat_public($row['id'])) echo 8;  else echo 12 ;?>">
                                    <a  class="list_link" href="<?php  echo url ?>/computer/list_view/<?php echo $row['id'] ?>" >
                                        <div class="row align-items-center">
                                            <div class="col-auto" style="padding-left: 3px" ><?php  echo $row['image']  ?> </div>
                                            <div class="col-<?php if ($this->ck_sub_cat_public($row['id'])) echo 8;else echo 10 ?>" style="padding-right: 3px;padding-left: 1px" >  <?php echo $row['title'] ?> </div>
                                        </div>
                                    </a>
                                </div>
...
//             echo true;
    ...
    public function like_d($id)
    {
        if (isset($_SESSION['username_member_r'])) {
            if (!is_numeric($id)) {$error=new Errors(); $error->index();}
            $stmt = $this->db->prepare("INSERT INTO  `{$this->like_computer}` (`id_device`,`id_member_r`,`like`,`date`) value (?,?,?,?)  ");
    ...
    public function unlike_d($id)
    {
        if (isset($_SESSION['username_member_r'])) {
            ...
            if (!is_numeric($id)) {$error=new Errors(); $error->index();}
            $stmt = $this->db->prepare("DELETE FROM  `{$this->like_computer}` WHERE `id_device`=? AND `id_member_r`=? ");
    ...
    { if (isset($_SESSION['username_member_r'])) {
        $stmt = $this->db->prepare("SELECT * FROM `{$this->like_computer}` WHERE `id_member_r` =?  AND `id_device` =  ? ");
    ...
    function range()
    {
        ...
        if ( abs(is_numeric($catRg)) &&  abs(is_numeric($catRg))  ) {
            ...
            $stmt = $this->db->prepare("SELECT *FROM `{$this->table}` WHERE `id`  IN ({$Id_item})   AND `active` = 1  AND `id_cat`  IN ({$Id_cat})  AND {$this->bast_it} ORDER BY `id` DESC  ");
        }else
        {
            $stmt = $this->db->prepare("SELECT *FROM `{$this->table}` WHERE `id`  IN ({$Id_item})   AND `active` = 1 AND {$this->bast_it} ORDER BY `id` DESC  ");
        }
        $stmt->execute();
    ...
    function class_computer($id,$id_cat=null)
    {
        ...
        }else
        {
            ...
            $stmt=$this->db->prepare("SELECT {$this->table}.id, {$this->table}.price_dollars,{$this->table}.title,{$this->table}.price_cuts,{$this->table}.bast_it,{$this->table}.cuts,{$this->table}.description  FROM {$this->table} INNER JOIN {$this->color} ON  {$this->color}.id_item={$this->table}.id INNER JOIN {$this->code} ON {$this->code}.id_color= {$this->color}.id INNER JOIN {$this->excel} ON {$this->excel}.code = {$this->code}.code WHERE   {$this->table}.`id_cat` IN({$id_category})  AND   {$this->table}.`class_computer`  REGEXP ? AND  {$this->table}.`active` = 1   AND {$this->table}.is_delete=0   group by   {$this->table}.`id`  ORDER BY {$this->table}.`id` DESC  ");
            $stmt->execute(array($q));
        }
    ...
    function list_class_computer()
    {
        $this->checkPermit('list_class_computer','games');
        $this->adminHeaderController($this->langControl('games'));

        require ($this->render($this->folder,'class_computer','list','php'));
        $this->adminFooterController();
    ...
    public function processing_class_computer()
    {

        $table = $this->class_computer;
        $primaryKey = 'id';
        ...
        $columns = array(
            ...
            array(
                ...
                'dt'        => 1,
                'formatter' => function($id, $row ) {
                    if ($this->permit('visible_class_computer','games')) {
                        return "
                <div style='text-align: center'>
                  <input {$this->ch_class_computer($id)} class='toggle-demo' onchange='visible_class_computer(this,$id)' type='checkbox' data-on='On' data-off='Off' id='toggle-event'    data-toggle='toggle' data-style='ios' data-onstyle='success' data-size='small'>
                 </div>
             ";
                    }
                    else
            ...
            array(
                ...
                'dt'        => 2,
                'formatter' => function($id, $row ) {
                    if ($this->permit('edit_class_computer','games')) {
                        return "
                   <div style='text-align: center;font-size: 23px;'>
                    <a href=" . url . "/" . $this->folder . "/edit_class_computer/$id> <i class='fa fa-pencil-square-o' aria-hidden='true'></i> </a>
                    </div> ";
                    }else
                    {
                        return "لا تمتلك صلاحية";
            ...
            array(
                ...
                'dt'        => 3,
                'formatter' => function($id, $row ) {
                    if ($this->permit('delete_class_computer','games')) {
                        return "
                <div style='text-align: center'>
                    <button class='btn class_delete_row'  data-toggle='modal' data-target='#exampleModal' data-id='{$id}' data-title='{$row[0]}'   >
                    <i class='fa fa-trash-o' aria-hidden='true'></i></i>
...
// SQL server connection information
    ...
    public function visible_class_computer($v_,$id_)
    {
        if ($this->handleLogin()) {
            ...
            $data = $this->db->update($this->class_computer, array('active' => $v), "`id`={$id}");
            echo 'true';
        }
    }


    function delete_class_computer($id)
    {
        if ($this->handleLogin() ) {
            $response = $this->db->delete($this->class_computer, "`id`={$id}");
            echo 'true';
        }
    }
    ...
    public function ch_class_computer($id)
    {

        $stmt = $this->db->prepare("SELECT * FROM $this->class_computer WHERE `id` = ? AND `active` = 1 ");
        $stmt->execute(array($id));
        if ($stmt->rowCount() > 0)
        {
            return 'checked';
    ...
    function add_class_computer()
    {
        $this->checkPermit('add_class_computer','games');
        ...
        if (isset($_POST['submit']))
        {
            try{
                ...
                {
                    $stmt_c=$this->db->prepare("INSERT INTO `{$this->class_computer}` (`title`,`userId`,`date`) VALUE (?,?,?)");
                    $stmt_c->execute(array($class, $this->userid ,time()));
                    $lastId=$this->db->lastInsertId();
                    $this->Add_to_sync_schedule($lastId,$this->class_computer,'add_class_computer');
                }

                $this->lightRedirect(url.'/'.$this->folder.'/list_class_computer',0);
            }
        ...
        require ($this->render($this->folder,'class_computer','add','php'));
        $this->adminFooterController();

    }

    function edit_class_computer($id)
    {

        if (!is_numeric($id)) { $error = new Errors(); $error->index();}
        $this->checkPermit('edit_class_computer','games');
        $data=$this->db->select("SELECT * from {$this->class_computer} WHERE `id`=:id LIMIT 1 ",array(':id'=>$id));
        $data=$data[0];
        ...
        if (isset($_POST['submit']))
        {
            try
            {
                ...
                $this->db->update($this->class_computer,$data,"id={$id}");
                $this->Add_to_sync_schedule($id,$this->class_computer,'add_class_computer');
                $this->lightRedirect(url.'/'.$this->folder.'/list_class_computer',0);
        ...
        require ($this->render($this->folder,'class_computer','edit','php'));
        $this->adminFooterController();
    ...
    public function list2_computer()
    {
...
// SQL server connection information
    ...
    public function processing_quantity($id)
    {
        ...
        $columns = array(
            ...
            array('db' => $this->excel.'.quantity', 'dt' => 5),
            array('db' => $this->excel.'.price_dollars', 'dt' => 6),
            array('db' =>$this->color.'.img', 'dt' => 7,
                'formatter' => function( $d, $row ) {
...
// SQL server connection information
    ...
    function index_data($id)
    {
        ...
        $stmt_content=$this->getAllcomputerFromContent($this->getLoopId($id),4);
        ...
        while ($row_cont = $stmt_content->fetch(PDO::FETCH_ASSOC)) {
            ...
            if (!empty($details)) {
                ...
                $category_computer[] = $row_cont;
            }

        }

        $category_computer=array_chunk($category_computer,4);
    ...
    public function getAllcomputerFromContent($id_cat = array(), $limit)
    {
    ...
    function load($id=null)
    {
        ...
        if (is_numeric($id)) {
            ...
            $stmt=$this->db->prepare("SELECT {$this->table}.id, {$this->table}.price_dollars,{$this->table}.title,{$this->table}.price_cuts,{$this->table}.bast_it,{$this->table}.cuts,{$this->table}.description  FROM {$this->table} INNER JOIN {$this->color} ON  {$this->color}.id_item={$this->table}.id INNER JOIN {$this->code} ON {$this->code}.id_color= {$this->color}.id INNER JOIN {$this->excel} ON {$this->excel}.code = {$this->code}.code   AND {$this->table}.is_delete=0  WHERE     {$this->table}.`active` = 1    group by   {$this->table}.`id`  ORDER BY {$this->table}.`id` DESC  LIMIT $position,$item_per_page");
            $stmt->execute();
        }
    ...
    function load_asc($id=null)
    {
        ...
        if (is_numeric($id)) {
            ...
            if ($stmt_ch->rowCount()>0)
            {
                $result2 = $stmt_ch->fetch(PDO::FETCH_ASSOC);
                $Id_cat=$result2['ids'];
                $stmt=$this->db->prepare("SELECT {$this->table}.id, {$this->table}.price_dollars,{$this->table}.title,{$this->table}.price_cuts,{$this->table}.bast_it,{$this->table}.cuts,{$this->table}.description  FROM {$this->table} INNER JOIN {$this->color} ON  {$this->color}.id_item={$this->table}.id INNER JOIN {$this->code} ON {$this->code}.id_color= {$this->color}.id INNER JOIN {$this->excel} ON {$this->excel}.code = {$this->code}.code WHERE     {$this->excel}.quantity > 0 AND   {$this->table}.`id_cat`  IN ({$Id_cat}) AND  {$this->table}.`active` = 1     AND {$this->bast_it}  AND {$this->is_delete} group by   {$this->table}.`id`  ORDER BY  CAST({$this->excel}.`price_dollars` as DECIMAL(25,15))    ASC    LIMIT $position,$item_per_page");
                $stmt->execute();
            ...
            $stmt=$this->db->prepare("SELECT {$this->table}.id, {$this->table}.price_dollars,{$this->table}.title,{$this->table}.price_cuts,{$this->table}.bast_it,{$this->table}.cuts,{$this->table}.description  FROM {$this->table} INNER JOIN {$this->color} ON  {$this->color}.id_item={$this->table}.id INNER JOIN {$this->code} ON {$this->code}.id_color= {$this->color}.id INNER JOIN {$this->excel} ON {$this->excel}.code = {$this->code}.code WHERE     {$this->table}.`active` = 1   AND {$this->is_delete}  group by   {$this->table}.`id`  ORDER BY  CAST({$this->excel}.`price_dollars` as DECIMAL(25,15))    ASC      LIMIT $position,$item_per_page");
            $stmt->execute();
        }
