...
class Groups extends Controller
{
      ...
      public function admin_category($id=0)
    {
        ...
        $this->checkPermit('View_category','groups');
        $this->adminHeaderController($this->langControl('groups'),$id);
    ...
    {
        if (!is_numeric($id)) {$error=new Errors(); $error->index();}
        $this->checkPermit('view_groups','groups');
        $this->adminHeaderController($this->langControl('view_groups'),$id);
        $breadcumbs = $this ->Breadcumbs( $this->category,$id);
        ...
        require ($this->render($this->folder,'html','list_groups','php'));
        $this->adminFooterController();
    ...
    {
        if (!is_numeric($id)) {$error=new Errors(); $error->index();}
        $stmt_groups=$this->getCategorygroups_not_Get_this_id($id);
        $cat_groups=array();
        ...
        {
            $stmt_v=$this->getgroupsByCatId($row['id']);
            $row['groups']=array();
            while ($row_content = $stmt_v->fetch(PDO::FETCH_ASSOC))
            {
                if ( $row_content['img'] !=0) {
                    $get_file = $this->db->select("SELECT * from `files` WHERE `id`=:id AND `module`=:module LIMIT 1 ", array(':id' => $row_content['img'], ':module' => 'groups'));
                    $get_file = $get_file[0];
                ...
                $row['groups'][]=$row_content;
            }
            $row['groups']=array_chunk($row['groups'],3);
            $cat_groups []=$row;
        }
    ...
    public  function ajax($id_cat=null,$page=0)
    {
        ...
        foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $key => $dt) {
            if ( $dt['img'] !=0) {
                $get_file = $this->db->select("SELECT * from `files` WHERE `id`=:id AND `module`=:module LIMIT 1 ", array(':id' => $dt['img'], ':module' => 'groups'));
                $get_file = $get_file[0];
    ...
    function  getCategorygroups_not_Get_this_id($id)
    {
    ...
    public  function details($id)
    {
        ...
        if (!empty($category)) {
            $stmt_groups = $this->getAllInfogContentLimit($category, 8);
            while ($row_groups= $stmt_groups->fetch(PDO::FETCH_ASSOC)) {

                    if ($row_groups['img'] !=0) {
                        $get_file= $this->db->select("SELECT * from `files` WHERE `id`=:id AND `module`=:module AND `lang`='{$this->langControl}' LIMIT 1 ", array(':id' => $row_groups['img'], ':module' => $this->folder));
                        if ($get_file)
                        {
                            $get_file=$get_file[0];
                            $row_groups['image'] = $this->save_file.$get_file['rand_name'];
                            $row_groups['type_file'] =$get_file['file_type'];
                        }else
                        {
                            $row_groups['type_file'] ='no-file';
                            $row_groups['image']=$this->static_file_control.'/image/admin/default.png';
                        }
                    }
                    else
                    {
        ...
        $sidebar=new Sidebar();
            ...
            while ($row_content = $stmt_v->fetch(PDO::FETCH_ASSOC))
            {
                ...
                $row_content['date']=date('Y-m-d',$row_content['date']);
                $new_groups[]=$row_content;
            }
            $new_groups_chunk=array_chunk($new_groups,4);
    ...
    public function add_groups($id=null)
    {
        if (!is_numeric($id)) {$error=new Errors(); $error->index();}
        $this->checkPermit('Add_groups','groups');
        $this->adminHeaderController($this->langControl('Add_groups'),$id);
        $breadcumbs = $this ->Breadcumbs( $this->category,$id);
        $data_cat=$this->db->select("SELECT * from  {$this->category} WHERE  `lang`='{$this->langControl}'");
        foreach ($data_cat as $key => $d_cat)
        {
            ...
            try
            {
             $form =new  Form();
                ...
                $file=new Files();
                  ...
                  $this->lightRedirect(url."/groups/list_groups/{$id}",0);
        ...
        require ($this->render($this->folder,'html','add_groups','php'));
        $this->adminFooterController();
    ...
    {
        $this->checkPermit('Edit_groups','groups');
        if (!is_numeric($id)) {
            die(':)');
        }
        ...
        $data=$this->db->select("SELECT * from `groups` WHERE `id`=:id AND `lang`='{$this->langControl}' LIMIT 1 ",array(':id'=>$id));
        $data=$data[0];
        $mainImg=$data['img'];
...
//                }
               ...
               $this->lightRedirect(url."/groups/list_groups/{$data['id_cat']}",0);
        ...
        require ($this->render($this->folder,'html','edit_groups','php'));
        $this->adminFooterController();
    ...
    $columns = array(
        ...
        array(
            ...
            'formatter' => function ($id, $row) {
                if ($this->permit('visible','groups')) {
                    return "
                <div style='text-align: center'>
                  <input {$this->ch($id)} class='toggle-demo' onchange='visible_groups(this,$id)' type='checkbox' data-on='On' data-off='Off' id='toggle-event'    data-toggle='toggle' data-style='ios' data-onstyle='success' data-size='small'>
                 </div>
             ";
                }
                else
        ...
        array(
            ...
            'dt' => 4,
            'formatter' => function ($id, $row) {
                if ($this->permit('edit','groups')) {
                    return "
                   <div style='text-align: center;font-size: 23px;'>
                    <a href=" . url . "/groups/edit_groups/$id> <i class='fa fa-pencil-square-o' aria-hidden='true'></i> </a>
                    </div> ";
                }
                else
                {
...
// SQL server connection information
    ...
    public function visible_groups($v_,$id_)
    {
        ...
        $data = $this->db->update("groups",array('active'=>$v), "`id`={$id} AND `lang`='{$this->langControl}'");
    }
    ...
    public function ch($id)
    {
        ...
        $stmt = $this->db->prepare("SELECT * FROM groups WHERE `id` = ? AND `active` = 1 AND `lang`='{$this->langControl}'");
        $stmt->execute(array($id));
        if ($stmt->rowCount() > 0)
        {
            return 'checked';
    ...
    public function select_category_groups($id)
    {
    ...
    public function select_category_groups_public($id)
    {
...
//-----------------category------------------
    ...
    {
        $this->checkPermit('add_category','groups');
        if (!is_numeric($id)){  $error=new Errors();  $error ->index();}
        $breadcumbs = $this ->Breadcumbs( $this->category,$id);
        ...
        if (isset($_POST['submit']))
        {
            try{
                ...
                $this->lightRedirect(url.'/groups/admin_category',0);
            }
    ...
    function edit_category($id)
    {
        ...
        $this->checkPermit('edit_category','groups');
...
//                     }
    ...
    public function getAllgroupsFromContent($id_cat = array(), $limit)
    {
        $Id_cat = implode(',', $id_cat);
