...
<script>
    $(document).ready(function() {
        ...
        $('#example').DataTable( {
            "processing": true,
            "serverSide": true,
            "ajax": "<?php echo url .'/'.$this->folder ?>/processing/<?php  echo $model ?>/<?php  echo $id ?>",
            info:true,
...
<form action="<?php  echo  url .'/'.$this->folder ?>/list_offers" method="get">
...
<div class="row">
    ...
    <div class="col-lg-9">
        ...
        <a  href="<?php echo url .'/'.$this->folder ?>/add" role="button"    class="btn btn-primary btn-sm"> <i class="fa fa-plus" aria-hidden="true"></i>  <span> اضافة عرض  </span> </a>
...
<script>
    ...
    function visible_range_price(e,id) {
        if (confirm('هل انت متأكد  ؟')) {
            var vis = $(e).is(':checked') ? 1 : 0;
            $.get("<?php echo url . '/' . $this->folder ?>/visible_range_price/" + vis + '/' + id, function (e) {
                if (e !== 'true') {
                    $('#toggle-offer_range_price' + id).prop('checked', false).change()
                    alert('حدث خطا')
                }
            })
        }else {
