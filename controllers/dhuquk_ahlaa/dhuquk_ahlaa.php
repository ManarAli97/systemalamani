...
class dhuquk_ahlaa extends Controller
{

    function __construct()
    {
        parent::__construct();
        $this->table='dhuquk_ahlaa';
        $this->category='category_dhuquk_ahlaa';
        $this->menu=new Menu();
...
//                     }
    ...
    public function details($id){
        ...
        $result=$stmtr->fetch(PDO::FETCH_ASSOC);
            ...
            $fileStmt = $this->db->prepare("SELECT rand_name FROM  files WHERE `relid` = ? AND module = 'dhuquk_ahlaa'   "); //will return all images except the primary image (squared).
    ...
    {
        $this->checkPermit('add_note','dhuquk_ahlaa');

        $note=strip_tags($_POST['dhuquk_ahlaa'],TAG);
        $stmt= $this->setting->set('dhuquk_ahlaa',$note);
    ...
    public function list_dhuquk_ahlaa($id)
    {
        $this->checkPermit('list_dhuquk_ahlaa','dhuquk_ahlaa');
        $this->adminHeaderController($this->langControl('dhuquk_ahlaa'));
    ...
    $columns = array(
        ...
        array(
            ...
            'formatter' => function($id, $row ) {
                if ($this->permit('visible','dhuquk_ahlaa')) {
                    return "
        ...
        array(
            ...
            'dt'        => 3,
            'formatter' => function($id, $row ) {
                if ($this->permit('edit','dhuquk_ahlaa')) {
                    return "
        ...
        array(
            ...
            'dt'        => 4,
            'formatter' => function($id, $row ) {
                if ($this->permit('delete','dhuquk_ahlaa')) {
                    return "
                <div style='text-align: center'>
...
// SQL server connection information
    ...
    public function visible_dhuquk_ahlaa($v_,$id_)
    {
        if ($this->handleLogin()) {
    ...
    function delete_dhuquk_ahlaa($id)
    {
        if ($this->handleLogin() ) {
    ...
    function add($id)
    {
        ...
        $this->checkPermit('add','dhuquk_ahlaa');
        ...
        if (isset($_POST['submit']))
        {
            try{
                ...
                $this->lightRedirect(url.'/'.$this->folder.'/list_dhuquk_ahlaa/'.$id,0);
            }
    ...
    function edit($id)
    {
        ...
        if (!is_numeric($id)) { $error = new Errors(); $error->index();}
        $this->checkPermit('edit','dhuquk_ahlaa');
        ...
        if (isset($_POST['submit']))
        {
            ...
            try
            {
                ...
                if(!empty($get_image))
                {
                    ...
                    $this->lightRedirect(url.'/'.$this->folder.'/list_dhuquk_ahlaa/'.$data['id_cat'],0);
                }else
                {
    ...
    public function getAlldhuquk_ahlaa($id)
    {
        if ($id==0)
        {
