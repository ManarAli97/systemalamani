        ...
        <?php  if (!empty($request)) { ?>
            <div class="row justify-content-between">
                <div class="col-auto infoBillOpen">
                   <span>رقم الفاتورة:</span>  <span    onclick="copy_text(this)"  class="copyToClipboard"   title="نسخ" data-clipboard-text="<?php  echo  $number_bill  ?>"  ><?php echo $number_bill ?></span> //         <span>  مجموع الفاتورة:</span>  <span    onclick="copy_text(this)"  id="number_bill_reload"  class="copyToClipboard"   title="نسخ" data-clipboard-text="<?php  echo  $price1  ?>" > <?php echo $price1 ?> </span><span>د.ع</span>
                ...
                <?php  		if ($this->permit('edit_bill', 'bills_inside_system')) {?>
                    <div class='row justify-content-center align-items-center'>
                        ...
                        <div class='col-auto' style='padding-left: 2px'>
                            <input   <?php  if (!empty($bills_crystal))  echo 'disabled'   ?>     style="width:200px " placeholder="رقم فاتورة كرستال" id='numberBill2_<?php echo $number_bill ?>'  value='<?php echo $bills_crystal   ?>' type='text' class='form-control withBill' name='crystal_bill' required>
                        </div>
                        <?php  if (empty($bills_crystal)) {   ?>
                        <div class='col-auto' style='padding-right: 2px'>
                            <button type='submit' id='btn_in_bill_<?php echo $number_bill ?>' onclick=saveBill2('<?php echo $number_bill ?>')  name='submit' class='btn btn-warning'>حفظ</button>
                        </div>
                        <?php  }   ?>
            ...
            <table class="requ_on table table-striped" border>
                ...
                <?php   foreach ($request as $rows)  {  ?>
                    <tr class="retn" id="row_<?php  echo $rows['id'] ?>">
                        ...
                        <td  onclick="copy_text(this)"  class="copyToClipboard"   title="نسخ" data-clipboard-text="<?php  echo  date('Y-m-d h:i:s A',$rows['date_accountant'])  ?>"><?php  echo date('Y-m-d h:i:s A',$rows['date_prepared']) ?></td>
