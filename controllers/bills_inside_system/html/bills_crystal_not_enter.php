    ...
    <div class="col">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item">  فواتير كرستال غير مدخلة  </li>
            </ol>
        </nav>
    ...
    $(document).ready(function() {
        ...
        $('#example thead tr:eq(1) th').each( function (i) {
          ...
          table= $('#example').DataTable( {
            "processing": true,
            "serverSide": true,
            "ajax": "<?php echo url .'/'.$this->folder ?>/processing_bills_crystal_not_enter/<?php   echo $from_date_stm .'/'.$to_date_stm  ?>",
            info:false,
...
<form action="<?php echo url.'/'.$this->folder?>/bills_crystal_not_enter" method="get">
...
<?php   if (!empty($bill)) {  ?>
    ...
    <button  id="export_g" class="btn btn-warning" onclick="add_group()" >   <span>   تصدير الفواتير الى مجموعة </span>  <span class="badge badge-info"> <?php  echo 'تصدير 100 من اصل '. count($bill) ?>  </span>
...
<script>
    ...
    function saveBill(number_bill,id) {
        ...
        {
                $.get( "<?php  echo url .'/'.$this->folder ?>/crystal_bill",{number_bill:number_bill,id:id,crystal_bill:$('#numberBill_'+number_bill+''+id).val()}, function( data ) {
                    if (data ==='1')
                    {
