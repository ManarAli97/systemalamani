...
class camera extends Controller
{
    ...
    function __construct()
    {
        parent::__construct();
        $this->table='camera';
        $this->category='category_camera';
        $this->color='color_camera';
        $this->code='code_camera';
        $this->excel='excel_camera';
        $this->cart_shop_active='cart_shop_active';
        $this->like_camera='like_camera';
        $this->menu=new Menu();
    ...
    public function createTB()
    {
        ...
        $this->db->query("CREATE TABLE IF NOT EXISTS `{$this->like_camera}` (
          `id` int(11)  NOT NULL AUTO_INCREMENT ,
        ...
        return  $this->db->cht(array($this->table, $this->category, $this->color, $this->code, $this->excel,$this->like_camera));
    ...
    public function admin_category($id=0)
    {
        ...
        $this->checkPermit('View_category','camera');
    ...
    public function list_camera($id=null)
    {
        if (!is_numeric($id)) {$error=new Errors(); $error->index();}
        $this->checkPermit('view_content','camera');
    ...
    public function add_camera($id=null,$r=null)
    {

        if (!is_numeric($id)) {$error=new Errors(); $error->index();}
        $this->checkPermit('add','camera');
               ...
               $form->post('is_service')
                ...
                if (empty($this->error_form)) {
                    ...
                    if ($r)
                    {
                        ...
                        $this->lightRedirect(url . "/camera/list_camera/{$id}", 0);
                    }
                }
            }catch (Exception $e)
            {
        ...
        if (isset($_POST['submit']))
        {
            ...
            try
            {
                ...
                if ($this->permit('save_edit',$this->folder)) {
                    ...
                    $this->lightRedirect(url . "/camera/list_camera/{$data['id_cat']}", 0);
        ...
        $columns = array(
            ...
            array(
                ...
                <div style='text-align: center'>
                  <input {$this->ch($id)} class='toggle-demo' onchange='visible_camera(this,$id)' type='checkbox' data-on='On' data-off='Off' id='toggle-event'    data-toggle='toggle' data-style='ios' data-onstyle='success' data-size='small'>
                 </div>
             ";  }
                    else
                    {
            ...
            array(
                ...
                'formatter' => function($id, $row ) {
                   ...
                   <div style='text-align: center;font-size: 23px;'>
                    <a href=".url."/camera/edit_camera/$id> <i class='fa fa-pencil-square-o' aria-hidden='true'></i> </a>
                    </div> ";
                }
            ),
            array( 'db' => 'id', 'dt' => 5,
...
// SQL server connection information
    ...
    public function visible_camera($v_,$id_)
    {
        if ($this->handleLogin()) {
    ...
    public function select_category_camera($id)
    {
...
//-----------------category------------------
    ...
    {
        $this->checkPermit('add_category','camera');
        if (!is_numeric($id)){  $error=new Errors();  $error ->index();}
        $breadcumbs = $this ->Breadcumbs( $this->category,$id);
        $this->adminHeaderController($this->lang('camera'),$id);
        $data['title']='';
                $data['code_cat']='';
    ...
    function edit_category($id)
    {
        ...
        $this->checkPermit('edit_category','camera');
    ...
    public function select_category_camera_public($id)
    {
    ...
    public function select_category_camera_publicAll()
    {
    ...
    public function listSubCategoryMenu_acc($relid)
    {
        ...
        while ($row = $stmt->fetch()) {
            ...
            if ($row['img'] !=0) {
                $get_file = $this->db->select("SELECT * from `files` WHERE `id`=:id AND `module`=:module LIMIT 1 ", array(':id' => $row['img'], ':module' => 'camera_cat'));
                $get_file = $get_file[0];
                $row['image'] ='<img class="ion_catg" src="'.$this->save_file.$get_file['rand_name'].'">';

            } else
            {
                $row['image']= '<i class="fa fa-video-camera"></i>';
            }
                        ...
                        <div class="card-header" id="headingOne1<?php echo $row['id'] ?>">
                            <div class="row justify-content-between align-items-center">
                                <div class="col-<?php if ($this->ck_sub_cat_public($row['id'])) echo 8;  else echo 12 ;?>">
                                    <a  class="list_link" href="<?php  echo url ?>/camera/list_view/<?php echo $row['id'] ?>" >
 ...
 function getAllItemByIdCat($id,$limit=5)
 {
    ...
    public function like_d($id)
    {
        if (isset($_SESSION['username_member_r'])) {
            if (!is_numeric($id)) {$error=new Errors(); $error->index();}
            $stmt = $this->db->prepare("INSERT INTO  `{$this->like_camera}` (`id_device`,`id_member_r`,`like`,`date`) value (?,?,?,?)  ");
    ...
    public function unlike_d($id)
    {
        if (isset($_SESSION['username_member_r'])) {
            ...
            if (!is_numeric($id)) {$error=new Errors(); $error->index();}
            $stmt = $this->db->prepare("DELETE FROM  `{$this->like_camera}` WHERE `id_device`=? AND `id_member_r`=? ");
    ...
    { if (isset($_SESSION['username_member_r'])) {
        $stmt = $this->db->prepare("SELECT * FROM `{$this->like_camera}` WHERE `id_member_r` =?  AND `id_device` =  ? ");
...
// SQL server connection information
        ...
        if ($id=='all' && empty($_GET['from_price']) && empty($_GET['to_price']) )
        {
            ...
            $whereAll = array("   {$this->is_delete} ");
        }else if ($id=='all' && !empty($_GET['from_price']) && !empty($_GET['to_price']) )
        {
            $join = " inner JOIN {$this->color} ON {$this->code}.id_color = {$this->color}.id  inner JOIN {$this->table} ON {$this->color}.id_item = {$this->table}.id LEFT JOIN {$this->excel} ON {$this->code}.code = {$this->excel}.code ";
            $whereAll = array("   {$this->is_delete} ","{$this->excel}.price_dollars BETWEEN {$_GET['from_price']} AND {$_GET['to_price']}");
        }else if (is_numeric($id)  && empty($_GET['from_price']) && empty($_GET['to_price']) )
        {
            ...
            $join = " inner JOIN {$this->color} ON {$this->code}.id_color = {$this->color}.id  inner JOIN {$this->table} ON {$this->color}.id_item = {$this->table}.id LEFT JOIN {$this->excel} ON {$this->code}.code = {$this->excel}.code ";
            $whereAll = array("   {$this->is_delete} ","{$this->table}.id_cat IN({$ids})");
        }else   if (is_numeric($id)  && !empty($_GET['from_price']) && !empty($_GET['to_price']) )
        {
            $ids=implode(',', $this->getLoopId($id));

            $join = " inner JOIN {$this->color} ON {$this->code}.id_color = {$this->color}.id  inner JOIN {$this->table} ON {$this->color}.id_item = {$this->table}.id LEFT JOIN {$this->excel} ON {$this->code}.code = {$this->excel}.code ";
            $whereAll = array("   {$this->is_delete} ","{$this->table}.id_cat IN({$ids})","{$this->excel}.price_dollars BETWEEN {$_GET['from_price']} AND {$_GET['to_price']}");
        }
...
// SQL server connection information
    ...
    function index_data($id)
    {
        ...
        $stmt_content=$this->getAllcameraFromContent($this->getLoopId($id),4);
        ...
        while ($row_cont = $stmt_content->fetch(PDO::FETCH_ASSOC)) {
            ...
            if (!empty($details)) {
                ...
                $category_camera[] = $row_cont;
            }

        }
        $category_camera=array_chunk($category_camera,4);
    ...
    {
        $stmt = $this->db->prepare("SELECT `{$this->color}`.*,  {$this->code}.size,  {$this->code}.code,   {$this->excel}.price_dollars, {$this->excel}.wholesale_price, {$this->excel}.wholesale_price2, {$this->excel}.cost_price, {$this->excel}.price  FROM `{$this->color}`  INNER JOIN {$this->code} ON {$this->code}.id_color = `{$this->color}`.id     INNER JOIN {$this->excel} ON {$this->excel}.code = `{$this->code}`.code    WHERE   `{$this->color}`.`id_item`= ? AND {$this->excel}.quantity > 0 ORDER BY  {$this->excel}.quantity  DESC LIMIT 1  ");
        $stmt->execute(array($id));
        return $stmt;
    }

    public function getAllcameraFromContent($id_cat = array(), $limit)
    {
    ...
    function load($id=null)
    {
    	...
    	//Array to show lable ('عروض كل يوم') on product model camera
        $offers_camera=array();
                        ...
                        if (count($rowData[0])  >= 2  ) {
                            ...
                            if ($stmtc->rowCount() > 0) {
                                ...
                                if ($stmt_point->rowCount() > 0 )
                                {
                                    ...
                                    $stmt = $this->db->prepare("SELECT  {$this->table}.* FROM {$this->table} INNER  JOIN  {$this->color} ON  {$this->color}.id_item  = {$this->table}.id  INNER  JOIN  {$this->code} ON  {$this->code}.id_color  = {$this->color}.id WHERE  {$this->code}.code=?  AND {$this->is_delete}  LIMIT 1");
                                    $stmt->execute(array($rowData[0][0] ));
                                    if ($stmt->rowCount() > 0) {
                                        $result = $stmt->fetch(PDO::FETCH_ASSOC);
                                        $trace=new trace_site();
