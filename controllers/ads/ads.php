...
class ads extends Controller
{
    ...
    {
        $this->checkPermit('list_ads','ads');
        $this->adminHeaderController($this->langControl('ads'));
...
// SQL server connection information
    ...
    public function visible_ads($v_,$id_)
    {
        if ($this->handleLogin()) {
    ...
    function add()
    {
        ...
        if (isset($_POST['submit']))
        {
            try{
                ...
                $this->lightRedirect(url.'/'.$this->folder.'/list_ads',0);
            }
...
//                     }
    ...
    public  function view_ads($page=1)
   {
    ...
    public  function details($id)
    {
        ...
        $stmt_ads=$this->db->prepare("SELECT *FROM `{$this->table}` WHERE `active` = ? AND `id` <> ? ORDER BY `id`DESC LIMIT 15");
        $stmt_ads->execute(array(1,$result['id']));
        $ads_content =array();
