...
<div class="row">
    <div class="col">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="<?php  echo url.'/'.$this->folder?>/list_ads"><?php  echo $this->langControl('ads') ?> </a></li>
...
<table id="example"  style="width:100%"   class="table table-striped display d-table"  >
    ...
    <tbody>
        ...
        <td>
           <?php  if ($this->permit('delete_services','special_offers')) { ?>
            <div style='text-align: center'>
...
<script>
    ...
    $('#save').on('click',function () {
        var  id= $('#save').val();
        $.get( "<?php echo url .'/'.$this->folder ?>/delete_ads/"+id, function( e ) {
            if (e !=='true')
            {
    ...
    function visible_news(e,id) {
        ...
        $.get("<?php echo url .'/'.$this->folder ?>/visible_ads/"+vis+'/'+id, function(e){
            if (e !=='true')
            {
                window.location='<?php  echo url ."/login/user"?>'
            }
