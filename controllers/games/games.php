...
class games extends Controller
{
    ...
    function __construct()
    {
        parent::__construct();
        $this->table='games';
        $this->category='category_games';
        $this->color='color_games';
        $this->code='code_games';
        $this->excel='excel_games';
        $this->cart_shop_active='cart_shop_active';
        $this->like_games='like_games';
        $this->class_games='class_games';
        $this->menu=new Menu();
    ...
    public function createTB()
    {
        ...
        $this->db->query("CREATE TABLE IF NOT EXISTS `{$this->table}` (
          ...
          `date` bigint(20) NOT NULL,
          `class_games` varchar(250) COLLATE utf8_unicode_ci NOT NULL,
           PRIMARY KEY (`id`)
     ) ENGINE=InnoDB AUTO_INCREMENT=0 DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci");
        ...
        $this->db->query("CREATE TABLE IF NOT EXISTS `{$this->like_games}` (
          `id` int(11)  NOT NULL AUTO_INCREMENT ,
        ...
        $this->db->query("CREATE TABLE IF NOT EXISTS `{$this->class_games}` (
           `id` int(10) NOT NULL AUTO_INCREMENT ,
           `title` varchar(250) COLLATE utf8_unicode_ci NOT NULL,
           `active` int(10) NOT NULL DEFAULT '0',
        ...
        return  $this->db->cht(array($this->table, $this->category, $this->color, $this->code, $this->excel,$this->like_games,$this->class_games));
    ...
    public function disk(){
        ...
        if (isset($_GET['g']))
        {
            ...
            if ($_GET['g'] == 0)
            {
                ...
                if (isset($_COOKIE['g_active'])) {
                    ...
                    unset($_COOKIE['g_active']);
                    setcookie('g_active', null, -1, '/');


                    if (isset($_COOKIE['id_user_disk']))
                    {

                    unset($_COOKIE['id_user_disk']);
                    setcookie('id_user_disk', null, -1, '/');
                    }


                    session_destroy();
                    header("Location:" . url); // games category disk
                }else
                {
                    header("Location:" . url); // games category disk
                }
        ...
        $cat=false;
        $stmt_disk=$this->db->prepare("SELECT *from  category_games WHERE  `id` =  ?  AND `active` = 1  AND {$this->is_delete} ");
        $stmt_disk->execute(array(62));

        if ($stmt_disk->rowCount()  >  0) {

            $cat=true;
            $result = $stmt_disk->fetch(PDO::FETCH_ASSOC);


            $stmt = $this->db->prepare("SELECT *FROM `category_games` WHERE `relid` =?   AND `active` = 1  AND {$this->is_delete}");
            $stmt->execute(array($result['id']));
            $cate_disk = array();
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                $row['sub_cat'] = array();
                $stmtsub = $this->db->prepare("SELECT *FROM `category_games` WHERE `relid` =?   AND `active` = 1  AND {$this->is_delete}");
                $stmtsub->execute(array($row['id']));
        ...
        require ($this->render($this->folder,'html','disk','php'));
    ...
    public function admin_category($id=0)
    {
        ...
        $this->checkPermit('View_category','games');
    ...
    public function list_games($id=null)
    {
        if (!is_numeric($id)) {$error=new Errors(); $error->index();}
        $this->checkPermit('view_content','games');
    ...
    public  function list_view($id=null,$page=1)
    {
        ...
        $class_games=$this->db->select("SELECT * from  {$this->class_games} WHERE `active`=1");
    ...
    public function details($id)
    {
        ...
        $id_c=0;
        $count=0;
        $g_c=$this->db->prepare("SELECT color_games.*,excel_games.quantity,excel_games.code FROM color_games INNER JOIN code_games ON code_games.id_color =color_games.id INNER JOIN excel_games ON excel_games.code =code_games.code WHERE color_games.id_item =? AND excel_games.quantity > 0");
    ...
    public function add_games($id=null,$r=null)
    {

        if (!is_numeric($id)) {$error=new Errors(); $error->index();}
        $this->checkPermit('add','games');
        ...
        $class_games=$this->db->select("SELECT * from  {$this->class_games} ");
       ...
       $data['is_service'] =0;
        if (isset($_POST['submit']))
        {
            try
            {
                ...
                $class_games=json_decode($data['class_games'],true);
                ...
                if (!empty($data['class_games'])) {
                    $class_games = json_decode($data['class_games'], true);
                    $class_games = implode(',', $class_games);
                }else{
                    $class_games='';
                }
                if (empty($this->error_form)) {
                    $stmt = $this->db->prepare("INSERT INTO `{$this->table}` (`title`,`id_cat`,`content`, `description` ,`date`,`bast_it`,`serial_flag`,`price_dollars`,`tags`,`class_games`,`specifications`,`cuts`,`price_cuts`,`location`,`enter_serial`,`change_price`,`userId`,`is_service`) VALUE (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
                    $stmt->execute(array($data['title'], $data['id_cat'], $data['content'],$data['description'], $data['date'], $data['bast_it'],$data['serial_flag'],$data['price_dollars'], $data['tags'], $class_games, $specifications, $data['cuts'], $data['price_cuts'], $data['location'], $data['enter_serial'], $data['change_price'],$this->userid,$data['is_service']));
                	...
                	$this->Add_to_sync_schedule($lastId,$this->folder,'add_games');
                    if ($r)
                    {
                        $this->lightRedirect(url . "/purchase_customer/purchase", 0);
                    }else {
                        $this->lightRedirect(url . "/games/list_games/{$id}", 0);
                    }
                }
            }catch (Exception $e)
            {
    ...
    public function edit_games($id)
    {
        ...
        $data = $this->db->select("SELECT * from `{$this->table}` WHERE `id`=:id AND  {$this->is_delete}  LIMIT 1 ", array(':id' => $id));
        $data = $data[0];
		$oldTitle=$data['title'];
        $breadcumbs = $this->Breadcumbs($this->category, $data['id_cat']);

        $clgame_id=explode(',',$data['class_games']);
        ...
        $stmt_color = $this->db->prepare("SELECT *FROM `{$this->color}` WHERE `id_item` = ? AND  {$this->is_delete}");
        ...
        while ($row = $stmt_color->fetch(PDO::FETCH_ASSOC))
        {
            ...
            $row['code']=array();
            $stmt_code = $this->db->prepare("SELECT *FROM `{$this->code}` WHERE `id_color` = ? AND  {$this->is_delete}");
            ...
            try
            {
              ...
              $form->post('is_service')
                ...
                if ($this->permit('save_edit',$this->folder)) {
                    if (empty($this->error_form)) {
                        $stmt = $this->db->prepare("UPDATE   `{$this->table}` SET  `title`=? ,  `content`=? ,`description`=? , `id_cat`=? ,`date`=? ,`bast_it`=? ,`serial_flag`=? ,`price_dollars`=?,`tags`=?,`class_games`=?,`specifications`=?,`cuts`=?,`price_cuts`=?,`location`=?,`enter_serial`=?,`change_price`=?,`userId`=?,`is_service`=? WHERE `id`=?");
                        $stmt->execute(array($data['title'], $data['content'],$data['description'], $data['id_cat'], $data['date'], $data['bast_it'], $data['serial_flag'], $data['price_dollars'], $data['tags'], $class_games, $specifications, $data['cuts'], $data['price_cuts'], $data['location'],$data['enter_serial'],$data['change_price'],$this->userid,$data['is_service'],$id));
                    	...
                    	$this->Add_to_sync_schedule($id,$this->folder,'add_games');
                        $this->lightRedirect(url . "/games/list_games/{$data['id_cat']}", 0);
                    }
                }
        ...
        $columns = array(
            ...
            array(
                ...
                <div style='text-align: center'>
                  <input {$this->ch($id)} class='toggle-demo' onchange='visible_games(this,$id)' type='checkbox' data-on='On' data-off='Off' id='toggle-event'    data-toggle='toggle' data-style='ios' data-onstyle='success' data-size='small'>
                 </div>
             ";  }
                    else
                    {
            ...
            array(
                ...
                'formatter' => function($id, $row ) {
                   ...
                   <div style='text-align: center;font-size: 23px;'>
                    <a href=".url."/games/edit_games/$id> <i class='fa fa-pencil-square-o' aria-hidden='true'></i> </a>
                    </div> ";
                }
            ),
            array( 'db' => 'id', 'dt' => 5,
...
// SQL server connection information
    ...
    function copy_row($id)
    {
        ...
        $stmt=$this->db->prepare("INSERT  INTO  games ( content, title, id_cat, id_main_cat, img, view, main_cat, date, bast_it, tags,class_games, specifications, cuts, price_cuts, description,  serial_flag, price_dollars, location, enter_serial, change_price,  is_delete,active,userId)  SELECT  content, title, id_cat, id_main_cat, img, view, main_cat, date, bast_it, tags,class_games, specifications, cuts, price_cuts, description, serial_flag, price_dollars, location, enter_serial, change_price, is_delete,0,$this->userid FROM games  WHERE id=?");
        $stmt->execute(array($id));
        if ($stmt->rowCount() > 0)
        {
    ...
    public function visible_games($v_,$id_)
    {
        if ($this->handleLogin()) {
    ...
    function delete_games($id)
    {
        if ($this->handleLogin()) {
            ...
            $this->Add_to_sync_schedule($id,'games','delete_item', $check_codes);
        	...
        	for($i = 0; $i < count($ids_color); $i++){
            	 $this->update_is_delete($this->code, 'id_color = '.$ids_color[$i].'');
    ...
    public function select_category_games($id)
    {
...
//-----------------category------------------
    ...
    {
        $this->checkPermit('add_category','games');
        if (!is_numeric($id)){  $error=new Errors();  $error ->index();}
        $breadcumbs = $this ->Breadcumbs( $this->category,$id);
        $this->adminHeaderController($this->lang('games'),$id);
        $data['title']='';
        $data['files']='';
        $data['order_cat']='';
        $data['code_cat']='';
        ...
        if (isset($_POST['submit']))
        {
            try{
				...
				//                 h27
					$this->Add_to_sync_schedule($id,'games','add_category');
                if(!empty($data['files'])) {
				...
				$this->Add_to_sync_schedule($id_cat_h,'games','add_category');
    ...
    function edit_category($id)
    {
        ...
        $this->checkPermit('edit_category','games');
        $data=$this->db->select("SELECT * from {$this->category} WHERE `id`=:id AND  {$this->is_delete} LIMIT 1 ",array(':id'=>$id));
        $data=$data[0];
        $breadcumbs = $this ->Breadcumbs( $this->category,$id);
        ...
        if ($this->handleLogin()) {
            ...
            if ($stmt_item->rowCount() > 0) {
                while ($row_item = $stmt_item->fetch()) {
                    ...
                    $stmt_codes = $this->db->prepare("SELECT id,code FROM `$this->code` where id_color in (select id from `$this->color` where id_item =? )");
            ...
            // $response = $this->db->delete($this->category,"`id`={$id}");
            $this->Add_to_sync_schedule($id, 'games', 'delete_category');
        }
    }
    ...
    public function select_category_games_public($id)
    {
    ...
    public function select_category_games_publicAll()
    {
    ...
    public function listSubCategoryMenu_acc($relid)
    {
        ...
        while ($row = $stmt->fetch()) {
            ...
            if ($row['img'] !=0) {
                $get_file = $this->db->select("SELECT * from `files` WHERE `id`=:id AND `module`=:module LIMIT 1 ", array(':id' => $row['img'], ':module' => 'games_cat'));
                $get_file = $get_file[0];
                $row['image'] ='<img class="ion_catg" src="'.$this->save_file.$get_file['rand_name'].'">';

            } else
            {
                $row['image']= '<i class="fa fa-gamepad"></i>';
            }
                        ...
                        <div class="card-header" id="headingOne1<?php echo $row['id'] ?>">
                            <div class="row justify-content-between align-items-center">
                                <div class="col-<?php if ($this->ck_sub_cat_public($row['id'])) echo 8;  else echo 12 ;?>">
                                    <a  class="list_link" href="<?php  echo url ?>/games/list_view/<?php echo $row['id'] ?>" >
...
// 			 $codes = implode(",",$check_codes);
        	 ...
        	 // $this->Add_to_sync_schedule($id,'games','delete_color_code', $codes); 
...
//             echo true;
    ...
    public function like_d($id)
    {
        if (isset($_SESSION['username_member_r'])) {
            if (!is_numeric($id)) {$error=new Errors(); $error->index();}
            $stmt = $this->db->prepare("INSERT INTO  `{$this->like_games}` (`id_device`,`id_member_r`,`like`,`date`) value (?,?,?,?)  ");
    ...
    public function unlike_d($id)
    {
        if (isset($_SESSION['username_member_r'])) {
            ...
            if (!is_numeric($id)) {$error=new Errors(); $error->index();}
            $stmt = $this->db->prepare("DELETE FROM  `{$this->like_games}` WHERE `id_device`=? AND `id_member_r`=? ");
    ...
    { if (isset($_SESSION['username_member_r'])) {
        $stmt = $this->db->prepare("SELECT * FROM `{$this->like_games}` WHERE `id_member_r` =?  AND `id_device` =  ? ");
    ...
    function class_games($id,$id_cat=null)
    {
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            if ($stmtIdItC->rowCount() > 0) {
                ...
                if ($this->loginUser()) {
                    ...
                    $row['price'] = $this->price_dollarsAdmin($details['price_dollars']) . ' د.ع ';

                } else {
                    if ($row['price_dollars'] == 1) {
                        $row['priceC'] = $this->price_dollars($details['price_dollars']);
                        $row['price'] = $this->price_dollars($details['price_dollars']) . ' د.ع ';
                    } else {
    ...
    function list_class_games()
    {
        $this->checkPermit('list_class_games','games');
        $this->adminHeaderController($this->langControl('games'));

        require ($this->render($this->folder,'class_games','list','php'));
        $this->adminFooterController();
    ...
    public function processing_class_games()
    {

        $table = $this->class_games;
        $primaryKey = 'id';
        ...
        $columns = array(
            ...
            array(
                ...
                'dt'        => 1,
                'formatter' => function($id, $row ) {
                    if ($this->permit('visible_class_games','games')) {
                        return "
                <div style='text-align: center'>
                  <input {$this->ch_class_games($id)} class='toggle-demo' onchange='visible_class_games(this,$id)' type='checkbox' data-on='On' data-off='Off' id='toggle-event'    data-toggle='toggle' data-style='ios' data-onstyle='success' data-size='small'>
                 </div>
             ";
                    }
                    else
            ...
            array(
                ...
                'dt'        => 2,
                'formatter' => function($id, $row ) {
                    if ($this->permit('edit_class_games','games')) {
                        return "
                   <div style='text-align: center;font-size: 23px;'>
                    <a href=" . url . "/" . $this->folder . "/edit_class_games/$id> <i class='fa fa-pencil-square-o' aria-hidden='true'></i> </a>
                    </div> ";
                    }else
                    {
                        return "لا تمتلك صلاحية";
            ...
            array(
                ...
                'dt'        => 3,
                'formatter' => function($id, $row ) {
                    if ($this->permit('delete_class_games','games')) {
                        return "
                <div style='text-align: center'>
                    <button class='btn class_delete_row'  data-toggle='modal' data-target='#exampleModal' data-id='{$id}' data-title='{$row[0]}'   >
                    <i class='fa fa-trash-o' aria-hidden='true'></i></i>
...
// SQL server connection information
    ...
    public function visible_class_games($v_,$id_)
    {
        if ($this->handleLogin()) {
            ...
            $data = $this->db->update($this->class_games, array('active' => $v), "`id`={$id}");
            echo 'true';
        }
    }


    function delete_class_games($id)
    {
        if ($this->handleLogin() ) {
            $response = $this->db->delete($this->class_games, "`id`={$id}");
            echo 'true';
        }
    }
    ...
    public function ch_class_games($id)
    {

        $stmt = $this->db->prepare("SELECT * FROM $this->class_games WHERE `id` = ? AND `active` = 1 ");
        $stmt->execute(array($id));
        if ($stmt->rowCount() > 0)
        {
            return 'checked';
    ...
    function add_class_games()
    {
        $this->checkPermit('add_class_games','games');
        ...
        if (isset($_POST['submit']))
        {
            try{
                ...
                foreach ($title as $key => $class)
                {
                    $stmt_c=$this->db->prepare("INSERT INTO `{$this->class_games}` (`title`,`userId`,`date`) VALUE (?,?,?)");
                    $stmt_c->execute(array($class, $this->userid ,time()));
                    $lastId=$this->db->lastInsertId();
                    $this->Add_to_sync_schedule($lastId,$this->class_games,'add_class_games');
                }

                 $this->lightRedirect(url.'/'.$this->folder.'/list_class_games',0);
            }
        ...
        require ($this->render($this->folder,'class_games','add','php'));
        $this->adminFooterController();

    }

    function edit_class_games($id)
    {

        if (!is_numeric($id)) { $error = new Errors(); $error->index();}
        $this->checkPermit('edit_class_games','games');
        $data=$this->db->select("SELECT * from {$this->class_games} WHERE `id`=:id LIMIT 1 ",array(':id'=>$id));
        $data=$data[0];
        ...
        if (isset($_POST['submit']))
        {
            try
            {
                ...
                $data =$form -> fetch();
                    ...
                    $this->db->update($this->class_games,$data,"id={$id}");
            		$this->Add_to_sync_schedule($id,$this->class_games,'add_class_games');
                    $this->lightRedirect(url.'/'.$this->folder.'/list_class_games',0);
        ...
        require ($this->render($this->folder,'class_games','edit','php'));
        $this->adminFooterController();
...
// SQL server connection information
    ...
    function category_name($id)
    {
        ...
        $stmt=$this->db->prepare("SELECT *from  {$this->category} WHERE  `id` = {$id}  ");
        $stmt->execute(array($id));
        $result=$stmt->fetch(PDO::FETCH_ASSOC);
        return $result['title'];
    }
    ...
    public function processing_quantity($id)
    {
        ...
        $columns = array(
            ...
            array('db' => $this->table.'.date', 'dt' => 5,
            'formatter' => function ($d, $row) {
                return date('Y-m-d h:i:s', $d);
            }

        ),
            array('db' => $tableJoin . 'serial', 'dt' =>6  ,
                'formatter' => function( $d, $row ) {
                    return  str_replace(',','-',$d);
                }
            ),
            array('db' => $this->excel.'.quantity', 'dt' => 7),
            array('db' => $this->excel.'.price_dollars', 'dt' => 8),
            array('db' =>$this->color.'.img', 'dt' => 9,
                'formatter' => function( $d, $row ) {
...
// SQL server connection information
        ...
        {
            $join = " inner JOIN {$this->color} ON {$this->code}.id_color = {$this->color}.id  inner JOIN {$this->table} ON {$this->color}.id_item = {$this->table}.id   inner JOIN {$this->category} ON {$this->category}.id = {$this->table}.id_cat        LEFT JOIN {$this->excel} ON {$this->code}.code = {$this->excel}.code ";
...
// SQL server connection information
    ...
    function index_data($id)
    {
        ...
        $stmt_content=$this->getAllgamesFromContent($this->getLoopId($id),4);
        ...
        while ($row_cont = $stmt_content->fetch(PDO::FETCH_ASSOC)) {
            ...
            if (!empty($details)) {
                ...
                $category_games[] = $row_cont;
            }

        }
        $category_games=array_chunk($category_games,4);
    ...
    public function getAllgamesFromContent($id_cat = array(), $limit)
    {
    ...
    function check_code()
    {
        ...
        {
            $code=trim($_GET['code']);
            $stmt=$this->db->prepare("SELECT *FROM {$this->code} WHERE code =? and is_delete =0");
            $stmt->execute(array($code));
            if ($stmt->rowCount() > 0)
            {
                echo '1';
