...
//                    $savers = new savers();
//                    $stmt = $this->db->prepare("SELECT  type_device.title as title_device,`product_savers`.*,excel_savers.price_dollars as excel_price_dollars,excel_savers.wholesale_price,excel_savers.wholesale_price2,excel_savers.cost_price  from `product_savers`  LEFT JOIN type_device ON type_device.id =product_savers.id_device   INNER  JOIN  excel_savers ON excel_savers.code = product_savers.code WHERE (product_savers.title LIKE ?  OR product_savers.tags LIKE ?  OR product_savers.code = ?) AND product_savers.`active`=1 AND  product_savers.`img` <> ''  AND  excel_savers.quantity > 0");
...
//
//						 $savers = new savers();
//						 $stmt = $this->db->prepare("SELECT  `product_savers`.`id` ,`product_savers`.`title` ,`product_savers`.latiniin from `product_savers`  LEFT JOIN type_device ON type_device.id =product_savers.id_device   INNER  JOIN  excel_savers ON excel_savers.code = product_savers.code WHERE (product_savers.title LIKE ?  OR product_savers.tags LIKE ?  OR product_savers.code = ?) AND product_savers.`active`=1 AND  product_savers.`img` <> ''  AND  excel_savers.quantity > 0  LIMIT 10");
...
//						 $mobile = new mobile();
//						 $Id_cat = implode(',', $mobile->getLoopId($id));
//						 $stmt = $this->db->prepare("SELECT `id`,`title`,`bast_it`,`price_dollars` FROM `mobile` WHERE ( title LIKE ? OR  tags LIKE ? ) AND `id_cat` IN ({$Id_cat}) AND `active` = 1 AND {$this->bast_it}  LIMIT 10");
...
//						 }
//
//						 $stmtcode = $this->db->prepare("SELECT mobile.`id`,mobile.`title`,mobile.`bast_it`,mobile.`price_dollars` FROM code inner JOIN color ON code.id_color = color.id inner JOIN mobile ON color.id_item = mobile.id WHERE code.code=?");
...
//
//						 $savers = new savers();
//                         $stmt = $this->db->prepare("SELECT  type_device.title as title_device,`product_savers`.`id` ,`product_savers`.`title`,`product_savers`.latiniin  from `product_savers`  LEFT JOIN type_device ON type_device.id =product_savers.id_device   INNER  JOIN  excel_savers ON excel_savers.code = product_savers.code WHERE (product_savers.title LIKE ?  OR product_savers.tags LIKE ?  OR product_savers.code = ?) AND product_savers.`active`=1 AND  product_savers.`img` <> ''  AND  excel_savers.quantity > 0  LIMIT 10");
...
//						 $accessories = new accessories();
//						 $Id_cat = implode(',', $accessories->getLoopId($id));
//						 $stmt = $this->db->prepare("SELECT `id`,`title`,`bast_it`,`price_dollars`  FROM `accessories` WHERE ( title LIKE ? OR  tags LIKE ? )  AND  `id_cat` IN ({$Id_cat})   AND `active` = 1 AND {$this->bast_it}  LIMIT 10");
//						 $stmt->execute(array($q, $q));
//						 if ($stmt->rowCount() > 0) {
//							 while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
...
//
//
//					 $stmtcode = $this->db->prepare("SELECT accessories.`id`,accessories.`title`,accessories.`bast_it`,accessories.`price_dollars` FROM color_accessories inner JOIN accessories ON color_accessories.id_item = accessories.id WHERE color_accessories.code=?");
//					 $stmtcode->execute(array(str_replace(' ','',$val)));
//					 if ($stmtcode->rowCount() > 0) {
//						 while ($row_code = $stmtcode->fetch(PDO::FETCH_ASSOC)) {
...
//						 $games = new games();
//						 $Id_cat = implode(',', $games->getLoopId($id));
//						 $stmt = $this->db->prepare("SELECT   `id`,`title`,`bast_it`,`price_dollars` FROM `games` WHERE ( title LIKE ? OR  tags LIKE ? )  AND  `id_cat` IN ({$Id_cat})    AND `active` = 1 AND {$this->bast_it} LIMIT 10");
//						 $stmt->execute(array($q, $q));
//
//						 if ($stmt->rowCount() > 0) {
//							 while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
...
//							 }
//						 }
//						 $stmtcode = $this->db->prepare("SELECT games.`id`,games.`title`,games.`bast_it`,games.`price_dollars` FROM code_games inner JOIN color_games ON code_games.id_color = color_games.id inner JOIN games ON color_games.id_item = games.id WHERE code_games.code=?");
//						 $stmtcode->execute(array(str_replace(' ','',$val)));
//						 if ($stmtcode->rowCount() > 0) {
//							 while ($row_code = $stmtcode->fetch(PDO::FETCH_ASSOC)) {
...
//						 $camera = new camera();
//						 $Id_cat = implode(',', $camera->getLoopId($id));
//						 $stmt = $this->db->prepare("SELECT  `id`,`title`,`bast_it`,`price_dollars` FROM `camera` WHERE ( title LIKE ? OR  tags LIKE ? )   AND  `id_cat` IN ({$Id_cat})  AND `active` = 1 AND {$this->bast_it} LIMIT 10");
//						 $stmt->execute(array($q, $q));
//
//						 if ($stmt->rowCount() > 0) {
//							 while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
...
//						 }
//
//						 $stmtcode = $this->db->prepare("SELECT camera.`id`,camera.`title`,camera.`bast_it`,camera.`price_dollars` FROM code_camera inner JOIN color_camera ON code_camera.id_color = color_camera.id inner JOIN camera ON color_camera.id_item = camera.id WHERE code_camera.code=?");
//						 $stmtcode->execute(array(str_replace(' ','',$val)));
//						 if ($stmtcode->rowCount() > 0) {
//							 while ($row_code = $stmtcode->fetch(PDO::FETCH_ASSOC)) {
...
//						 $printing_supplies = new printing_supplies();
//						 $Id_cat = implode(',', $printing_supplies->getLoopId($id));
//						 $stmt = $this->db->prepare("SELECT  `id`,`title`,`bast_it`,`price_dollars` FROM `printing_supplies` WHERE ( title LIKE ? OR  tags LIKE ? )   AND  `id_cat` IN ({$Id_cat})  AND `active` = 1 AND {$this->bast_it} LIMIT 10");
//						 $stmt->execute(array($q, $q));
//
//						 if ($stmt->rowCount() > 0) {
//							 while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
...
//						 $computer = new computer();
//						 $Id_cat = implode(',', $computer->getLoopId($id));
//						 $stmt = $this->db->prepare("SELECT  `id`,`title`,`bast_it`,`price_dollars` FROM `computer` WHERE ( title LIKE ? OR  tags LIKE ? )   AND  `id_cat` IN ({$Id_cat})  AND `active` = 1 AND {$this->bast_it} LIMIT 10");
//						 $stmt->execute(array($q, $q));
//
//						 if ($stmt->rowCount() > 0) {
//							 while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
...
//						 }
//
//						 $stmtcode = $this->db->prepare("SELECT computer.`id`,computer.`title`,computer.`bast_it`,computer.`price_dollars` FROM code_computer inner JOIN color_computer ON code_computer.id_color = color_computer.id inner JOIN computer ON color_computer.id_item = computer.id WHERE code_computer.code=?");
//						 $stmtcode->execute(array(str_replace(' ','',$val)));
//						 if ($stmtcode->rowCount() > 0) {
//							 while ($row_code = $stmtcode->fetch(PDO::FETCH_ASSOC)) {
...
//						 $network = new network();
//						 $Id_cat = implode(',', $network->getLoopId($id));
//						 $stmt = $this->db->prepare("SELECT `id`,`title`,`bast_it`,`price_dollars`  FROM `network` WHERE ( title LIKE ? OR  tags LIKE ? )   AND  `id_cat` IN ({$Id_cat})   AND `active` = 1 AND {$this->bast_it} LIMIT 10");
//						 $stmt->execute(array($q, $q));
//						 if ($stmt->rowCount() > 0) {
//							 while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
...
//						 }
//
//						 $stmtcode = $this->db->prepare("SELECT network.`id`,network.`title`,network.`bast_it`,network.`price_dollars` FROM code_network inner JOIN color_network ON code_network.id_color = color_network.id inner JOIN network ON color_network.id_item = network.id WHERE code_network.code=?");
//						 $stmtcode->execute(array(str_replace(' ','',$val)));
//						 if ($stmtcode->rowCount() > 0) {
//							 while ($row_code = $stmtcode->fetch(PDO::FETCH_ASSOC)) {
