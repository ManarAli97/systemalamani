...
class printing_supplies extends Controller
{
    ...
    {
        parent::__construct();
        $this->table='printing_supplies';
        $this->category='category_printing_supplies';
        $this->color='color_printing_supplies';
        $this->code='code_printing_supplies';
        $this->excel='excel_printing_supplies';
        $this->cart_shop_active='cart_shop_active';
        $this->like_printing_supplies='like_printing_supplies';
        $this->menu=new Menu();
    ...
    public function createTB()
    {
        ...
        $this->db->query("CREATE TABLE IF NOT EXISTS `{$this->like_printing_supplies}` (
          `id` int(11)  NOT NULL AUTO_INCREMENT ,
        ...
        return  $this->db->cht(array($this->table, $this->category, $this->color, $this->code, $this->excel,$this->like_printing_supplies));
    ...
    public function admin_category($id=0)
    {
        ...
        $this->checkPermit('View_category','printing_supplies');
    ...
    public function list_printing_supplies($id=null)
    {
        if (!is_numeric($id)) {$error=new Errors(); $error->index();}
        $this->checkPermit('view_content','printing_supplies');
    ...
    public function add_printing_supplies($id=null,$r=null)
    {

        if (!is_numeric($id)) {$error=new Errors(); $error->index();}
        $this->checkPermit('add','printing_supplies');
            ...
            try
            {
                ...
                if (empty($this->error_form)) {
                    ...
                    foreach ($name_color as $key => $save_data) {
                        ...
                        if ($stmt_c->rowCount() > 0) {
                            ...
                            foreach ($code[$key] as $index => $cd) {
                                ...
                                $stmt_cd->execute(array($cd, $size[$key][$index], $point[$key][$index], $serial[$key][$index], $lastId_c, time()));
                            }
                        }
                    ...
                    if ($r)
                    {
                        ...
                        $this->lightRedirect(url . "/printing_supplies/list_printing_supplies/{$id}", 0);
                    }
                }
            }catch (Exception $e)
            {
    ...
    public function edit_printing_supplies($id)
    {
        $this->checkPermit('edit', 'printing_supplies');
        if (!is_numeric($id)) {
            $error = new Errors();
            $error->index();
        }
            ...
            try
            {
               ...
               $form->post('is_service')
                ...
                if ($this->permit('save_edit',$this->folder)) {
                    ...
                    $this->lightRedirect(url . "/printing_supplies/list_printing_supplies/{$data['id_cat']}", 0);
        ...
        $columns = array(
            ...
            array(
                ...
                <div style='text-align: center'>
                  <input {$this->ch($id)} class='toggle-demo' onchange='visible_printing_supplies(this,$id)' type='checkbox' data-on='On' data-off='Off' id='toggle-event'    data-toggle='toggle' data-style='ios' data-onstyle='success' data-size='small'>
                 </div>
             ";  }
                    else
                    {
            ...
            array(
                ...
                'formatter' => function($id, $row ) {
                   ...
                   <div style='text-align: center;font-size: 23px;'>
                    <a href=".url."/printing_supplies/edit_printing_supplies/$id> <i class='fa fa-pencil-square-o' aria-hidden='true'></i> </a>
                    </div> ";
                }
            ),
            array( 'db' => 'id', 'dt' => 5,
...
// SQL server connection information
    ...
    public function visible_printing_supplies($v_,$id_)
    {
        if ($this->handleLogin()) {
    ...
    function delete_printing_supplies($id)
    {
        if ($this->handleLogin()) {
    ...
    public function select_category_printing_supplies($id)
    {
    ...
    public function sub_cat_active($r_id,$id=null)
    {
        ...
        if ($id==null)
        {
            return 'close_this_printing_supplies';
        }
...
//-----------------category------------------
    ...
    {
        $this->checkPermit('add_category','printing_supplies');
        if (!is_numeric($id)){  $error=new Errors();  $error ->index();}
        $breadcumbs = $this ->Breadcumbs( $this->category,$id);
        $this->adminHeaderController($this->lang('printing_supplies'),$id);
        $data['title']='';
                $data['code_cat']='';
    ...
    function edit_category($id)
    {
        ...
        $this->checkPermit('edit_category','printing_supplies');
    ...
    public function select_category_printing_supplies_public($id)
    {
    ...
    public function select_category_printing_supplies_publicAll()
    {
    ...
    public function listSubCategoryMenu_acc($relid)
    {
        ...
        while ($row = $stmt->fetch()) {
            ...
            if ($row['img'] !=0) {
                $get_file = $this->db->select("SELECT * from `files` WHERE `id`=:id AND `module`=:module LIMIT 1 ", array(':id' => $row['img'], ':module' => 'printing_supplies_cat'));
                $get_file = $get_file[0];
                $row['image'] ='<img class="ion_catg" src="'.$this->save_file.$get_file['rand_name'].'">';

            } else
            {
                $row['image']= '<i class="fa fa-video-printing_supplies"></i>';
            }
                        ...
                        <div class="card-header" id="headingOne1<?php echo $row['id'] ?>">
                            <div class="row justify-content-between align-items-center">
                                <div class="col-<?php if ($this->ck_sub_cat_public($row['id'])) echo 8;  else echo 12 ;?>">
                                    <a  class="list_link" href="<?php  echo url ?>/printing_supplies/list_view/<?php echo $row['id'] ?>" >
 ...
 function getAllItemByIdCat($id,$limit=5)
 {
    ...
    function car_item($id,$count)
    {
        ...
        if (empty($error))
        {
            ...
            if ($stmt_ch->rowCount() > 0) {
                ...
                $stmt_order = $this->db->prepare("SELECT   SUM(`number`)as num,`dollar_exchange`  FROM `cart_shop_active` WHERE `code` =?  AND  (`buy` = 0 OR `buy` = 1) AND `status` = 0    AND `table`=?  AND  `id_member_r` = ?");
    ...
    public function like_d($id)
    {
        if (isset($_SESSION['username_member_r'])) {
            if (!is_numeric($id)) {$error=new Errors(); $error->index();}
            $stmt = $this->db->prepare("INSERT INTO  `{$this->like_printing_supplies}` (`id_device`,`id_member_r`,`like`,`date`) value (?,?,?,?)  ");
    ...
    public function unlike_d($id)
    {
        if (isset($_SESSION['username_member_r'])) {
            ...
            if (!is_numeric($id)) {$error=new Errors(); $error->index();}
            $stmt = $this->db->prepare("DELETE FROM  `{$this->like_printing_supplies}` WHERE `id_device`=? AND `id_member_r`=? ");
    ...
    { if (isset($_SESSION['username_member_r'])) {
        $stmt = $this->db->prepare("SELECT * FROM `{$this->like_printing_supplies}` WHERE `id_member_r` =?  AND `id_device` =  ? ");
    ...
    public function list2_printing_supplies()
    {
...
// SQL server connection information
	...
	function  unknown()
	{
		...
		$data_cat = $this->db->select("SELECT * from  {$this->category} WHER {$this->is_delete} ");
		foreach ($data_cat as $key => $d_cat) {
...
// SQL server connection information
    ...
    function search()
    {
        if ($this->handleLogin()) {
            $data = $_GET['q'];
            $data = '%' . $data . '%';
            $stmt = $this->db->prepare("SELECT * FROM `{$this->category}`  WHERE title LIKE ?    ");
    ...
    function index_data($id)
    {
        ...
        $stmt_content=$this->getAllprinting_suppliesFromContent($this->getLoopId($id),4);
        ...
        while ($row_cont = $stmt_content->fetch(PDO::FETCH_ASSOC)) {
            ...
            if (!empty($details)) {
                ...
                $category_printing_supplies[] = $row_cont;
            }

        }
        $category_printing_supplies=array_chunk($category_printing_supplies,4);
    ...
    public function getAllprinting_suppliesFromContent($id_cat = array(), $limit)
    {
