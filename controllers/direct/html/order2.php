...
<!--                            <div class="col-auto">-->
<!--                                <button class="btn btn-danger" onclick="cancel_order(    this->userid  ,   $date_done['number_bill']   )"> الغاء الفاتورة </button>-->
<!--                            </div>-->
...
<!--                                            تم ايقافة بسبب امكانية المحاسب الرئيسي    يستطيع محاسبة فواتير المحاسب الثانوي وئوثر على الحسابات يمكن ان يستخدمخاصية المرتجع-->
 										<?php  } else{ ?>
                                            مسترجع
										<?php  } ?>
...
<script>
    function  return_order_minus3After_pay(id_order,table,colde,id_user,color,number_bill,id) {

        $.get( "<?php  echo url .'/'.$this->folder?>/check_money_oue_item/" + table + "/" + colde + "/" + id_user, {color: color, number_bill: number_bill, id: id}, function( data ) {
            ...
            if (data==='true') {
                ...
                if ($('#number_item_' + id_order).text() === '1') {
                    if (confirm('  هل تريد استرجاع اخر مادة الى المخزن ؟ ')) {
                        ...
                        $.ajax({
                            type: 'GET',
                            url: "<?php  echo url . '/' . $this->folder ?>/return_order_minus3After_pay/" + table + "/" + colde + "/" + id_user,
                            cache: false,
                            data: {color: color, number_bill: number_bill, id: id},
            ...
            }else
            {
                ...
                alert('لايوجد رصيد في حسابك لسترجاع المبلغ بعد الغاء الطلب')
    ...
    function cancel_order(id,n_bill) {
        ...
        {
            $.get( "<?php  echo url .'/'.$this->folder?>/check_money/<?php echo $result['id'] ?>/"+n_bill, function( data ) {

                if (data==='true')
                {
                    $.get( "<?php  echo url .'/'.$this->folder?>/cancel_order/"+id+"/"+n_bill, function( data ) {
