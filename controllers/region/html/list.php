...
<div class="row align-items-center">
    <div class="col">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="<?php  echo url.'/'.$this->folder?>"><?php  echo $this->langControl('region') ?> </a></li>
...
<div class="row">
    <div class="col-lg-9">
        <a  href="<?php echo url .'/'.$this->folder   ?>/add_region" role="button"    class="btn btn-primary btn-sm"> <i class="fa fa-plus" aria-hidden="true"></i>  <span><?php  echo $this->langControl('add_content') ?>  </span> </a>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        ...
        $('#example').DataTable( {
            "processing": true,
            "serverSide": true,
            "ajax": "<?php echo url .'/'.$this->folder ?>/processing_region",
            info:false,
...
<script>
    ...
    $('#save').on('click',function () {
        var  id= $('#save').val();
        $.get( "<?php echo url .'/'.$this->folder ?>/delete_region/"+id, function( data ) {
            $('#row_'+id).remove();
...
<script>
    ...
    function visible_region(e,id) {
        var vis=$(e).is( ':checked' )? 1:0;
        $.get("<?php echo url .'/'.$this->folder ?>/visible_region/"+vis+'/'+id, function(){ })
    }
...
<script>
    $('#exampleModal_edit').on('show.bs.modal', function (event) {
        ...
        var modal = $(this);
        $.ajax({
            url: "<?php  echo url . '/' . $this->folder?>/get_region/"+id,
            cache: false,
            success: function(data){
                if (data)
                {
                    ...
                    modal.find('#edit_region').attr("action","<?php  echo url .'/'.$this->folder?>/edit_region/"+id);
                }
            }
        });
    });
